...
def _resolve_space_owner(spec: SpaceSpec, identity: Scope) -> SpaceSpec:
    """定妥建空间时的归属登记项，三种形态各走一条分支（S09「空间开通契约」）。
...
def _truthy_metadata(metadata: dict[str, Any] | None, key: str) -> bool:
    """写入路径开关的取值判定，与引擎侧同口径（``infer`` / ``procedural`` 等）。"""
    return str((metadata or {}).get(key, "")).strip().lower() in {"1", "true", "yes", "on"}


def _reject_route_tag_keys(metadata: dict[str, Any] | None, tag_keys: frozenset[str]) -> None:
    """写入边界拒绝调用方占用判定标签键（S09「写入边界校验」）。
...
def _write_permission_context(
    ...
    对 ``_TRANSIENT_EXTENSION_KEYS`` 的处理同向——瞬态键不越过持久化与判定边界。
    """
    meta = {
        key: str(value)
...
def _routing_clauses_of(
    ...
    单空间与跨空间两个检索入口共用本函数：只挂一处时未挂的那个即该绑定的绕过通道，而
    ``search_spaces`` 在显式传 ``spaces`` 时不依赖反查索引，路径始终可达。
    """
    clauses: list[FilterClause] = []
    for field in routing_fields:
...
def _first_family_predicate(actor: Scope, context: PermissionContext | None) -> list[FilterClause]:
    """检索谓词第一族：个体空间的系统谓词（S09「检索两族谓词」）。

    实现在 :func:`collective.individual_space_predicates`，本函数只从权限上下文里取出
    空间事实。两族谓词的生成规则集中在 ``collective``，因为跨空间检索侧要用同一份规则——
    两处各写一份即出现「单空间搜不到、跨空间搜得到」。
    """
    facts = context.space_facts if context is not None else None
    return collective.individual_space_predicates(facts, actor)
...
def _trim_space_policy(info: SpaceInfo, axis: str) -> SpaceInfo:
    """按通过的轴裁剪空间策略（S09「空间策略必须从元数据返回值中裁剪」）。
...
class LocalMemoryAPI(MemoryAPI):
    ...
    def _can_write_space(self, identity: Scope, target: Scope) -> bool:
    ...
    def _route_context(self, identity: Scope, coords: dict[str, str] | None) -> RouteContext:
        ...
        fallback 空间不可写时 :func:`collective.plan_write_targets` 整体拒绝写入——判不准
        时无处可落，静默落到别处等于把兜底落点交给判定实现决定。
        """
        table = self._route_table
        resolved = collective.kernel_coords(coords, identity)
        self._ensure_fallback_space(identity, identity.org, table.naming.fallback_space(resolved))
        targets = collective.plan_write_targets(
            identity.org,
            resolved,
            table.naming,
            can_write=lambda scope: self._can_write_space(identity, scope),
    ...
    def _ensure_space_state_allows(
        ...
        *,
        ...
        方向是更严格：冻结空间的运维介入须先解冻。见 F03 决策 4「五步无判据来源」。
        """
    ...
    async def add_async(
        ...
        *,
        ...
        collective.reject_kernel_coords(coords)
    ...
    def _write_target(
        ...
        | 调用形态 | 落点 | 判定 |
        |---|---|---|
        | 传了 ``scope`` | 就是它（启用判定时归一为空间级两维） | 否 |
        | 省略 ``scope``、``infer``/``procedural`` 为真 | fallback 空间作载体 | 在构建层 |
        | 省略 ``scope``、其余情形 | 判定算子在候选集内选 | 在本层，整条内容作一条候选送判 |
    ...
    def _explicit_scope_target(
        self, scope: Scope, identity: Scope, metadata: dict[str, Any] | None
    ) -> tuple[Scope, dict[str, Any] | None]:
    ...
    def _routed_targets(
        ...
        """省略 ``scope`` 的写入：整批一次判定，逐项产出落点与要补写的判定产物。
        ...
        **判定上下文每批只算一次。** ``coords`` 与 ``identity`` 批内恒定，候选空间集合与
        ...
        理由见 :func:`collective.route_many`。
        """
        if not entries:
            return {}
        if not self._routing_enabled():
            error = ValidationError("scope is required: no routing table is configured")
    ...
    def _normalize_batch_item(
        ...
        *,
        ...
        """归一一项批量写入；两级 ``scope`` 皆空时把落点留给归属判定。

        「逐项 scope 与批级 scope 皆为 None 即缺参」这条校验必须让位于判定，否则批量入口的
        省略 ``scope`` 永远进不到判定，``coords`` 加在 ``batch_add_async`` 上即无实际作用。
        未装配判定算子时该校验照旧生效——判定路径不可达，缺参就是缺参。
        """
        ...
        if target_scope is None and self._routing_enabled():
            pass  # 落点由本项的归属判定给出，在调用处解析
        elif not isinstance(target_scope, Scope):
    ...
    async def batch_add_async(
        ...
        *,
        ...
        # 第 1 遍：规范化。落点解析不在这一遍——省略 scope 的项要整批一次送判，逐项各判
    ...
    def search(
        ...
        *,
        ...
        system_clauses = collective.system_predicates(
            auth_context.space_facts if auth_context is not None else None,
            identity,
            collective.narrow_from_coords(
                coords or {}, self._route_table.narrow_dims, identity
            ),
        )
        if system_clauses:
    ...
    def search_spaces(
        ...
        *,
        ...
        """跨空间检索：五步编排，底层仍是既有的单空间召回（S09「多空间读写」）。

        第 2 步（逐空间判权）不放进协程：事实读取与判权都是同步调用，授权记录查询还会在
        存储层串行。第 3 步给的是每空间取数上界而非定额（理由见
        :func:`collective.allocate_quota`），总取数由候选空间上限与
        :data:`collective.TOTAL_FETCH_CAP` 两道封顶；判权前置的副产品是上界按实际可读空间
        数计算，无权空间不再凭空占用召回名额。
        """
        principal.require_principal(identity)
        collective.reject_kernel_coords(coords)
        ...
        narrow = collective.narrow_from_coords(
            coords or {}, self._route_table.narrow_dims, identity
        )

        candidates = self._search_candidates(identity, org, spaces)
        readable: list[tuple[Scope, PermissionContext]] = []
        for space in candidates:
            target = Scope(org=org, space=space)
            permission_context, _facts = self._apply_space_policy_context(
                target,
            ...
            try:
                ...
                continue
            if outcome.allowed:
                readable.append((target, permission_context))

        quota = collective.allocate_quota([scope.space for scope, _ in readable], top_k)
        results = asyncio.run(
            self._recall_spaces(
                query,
                readable,
        ...
        merged = collective.merge(
            results, top_k=top_k, priority=[scope.space for scope, _ in readable]
        )
        self._log(
            identity,
            "search",
            ...
            detail={
                ...
                "readable_spaces": str(len(readable)),
    ...
    def _search_candidates(
        self, identity: Scope, org: str, spaces: list[str] | None
    ) -> list[str]:
        """第 1 步：定候选空间。``spaces`` 给了就用它，否则取主体反查索引结果。
    ...
    async def _recall_spaces(
        ...
        *,
        ...
        """第 4 步：每空间一次召回，带自己的配额与两族谓词，并发度由候选集上限约束。

        **实际并发度取决于引擎实现。** 本方法按并发写就，但 ``Engine.recall`` 在
        ``CloudEngine`` 与 ``InMemoryEngine`` 中都是不含 ``await`` 的 ``async def``——
        内部的 ``retriever.retrieve`` 是同步阻塞调用，``gather`` 因而顺序执行，总时延是
        空间数乘以单空间时延。这是引擎侧的既有形态，不由本特性引入；要拿到真并发需
        ``asyncio.to_thread`` 包住每空间召回（前提是确认检索器与存储客户端多线程共用
        安全），或由引擎层提供真正异步的 ``recall``。两者都属引擎层改动。

        单个空间失败不使整次调用失败：跨空间检索的语义是「在能读的空间里找」，一个后端
        故障导致整次返回空，与「这些空间里没有内容」不可区分。失败空间记一条通道错误。
        """

        async def _one(target: Scope, permission_context: PermissionContext) -> tuple[
            str, RetrievalResult
        ]:
            rq = RetrievalQuery(
                text=query,
                filters=filters,
                as_of=as_of,
                top_k=quota.get(target.space, 1),
                disclosure=disclosure,
                max_tokens=max_tokens,
                with_trajectory=with_trajectory,
                extensions=dict(options),
            )
            # 与单空间入口同一处理：授权所依据的路由值回注，再叠两族系统谓词。逐空间
            # 各回注自己的那份——各空间的授权可以来自不同的策略。
            clauses = _routing_clauses_of(permission_context, self._perm.routing_fields())
            clauses.extend(
                collective.system_predicates(permission_context.space_facts, identity, narrow)
            )
            if clauses:
                rq.filters = and_merge(rq.filters, clauses)
            return target.space, await self._engine.recall(target, rq)

        gathered = await asyncio.gather(
            *(_one(target, ctx) for target, ctx in readable), return_exceptions=True
        )
        results: list[tuple[str, RetrievalResult]] = []
        for (target, _ctx), outcome in zip(readable, gathered):
            if isinstance(outcome, BaseException):
                self._log(
                    ...
                    target_scope=target,
                    decision="error",
                    detail={"entry": "search_spaces", "error": str(outcome)},
                )
                # 失败空间产出一条通道错误交由 merge 归并：只记审计日志时调用方无从区分
                # 「这个空间里没有内容」与「这个空间挂了」，而两者的后续动作完全不同。
                results.append(
                    (
                        target.space,
                        RetrievalResult(
        ...
        *,
        ...
        if self._needs_space_facts():
            ...
            if cursor is not None:
                # S09「``cursor`` 标记废弃」：``limit`` 在鉴权之后生效，偏移量按候选序解释
    ...
    def _listable_candidates(self, org: str, status: SpaceStatus | None) -> list[SpaceInfo]:
        ...
        **不取主体反查索引。** 索引的超集契约针对成员关系——写入方只有归属登记与成员记录
        ...
        直接 ``search`` 读得到、``list_spaces`` 却列不出来，且不报错。与 F03 决策 23
        ...
        方向。前置条件与遗留事项见 S09「``list_spaces`` 的候选来源」。
