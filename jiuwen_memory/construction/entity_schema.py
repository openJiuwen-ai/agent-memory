...
Schema JSON 的根节点既可为 entity type 数组，也可为
``{"entity_types": [...]}``；属性定义保留原始 dict，以便 prompt 完整携带
``desc/example/order`` 等扩展字段。
"""
...
class EntitySchemaCatalog:
    ...
    def filter_selected(
        ...
        """按 schema-selection 结果过滤，并始终保留 default_property。"""
        ...
        for item in selected:
            ...
            if not entity_type:
                continue
            properties = item.get("relevant_properties")
            if not isinstance(properties, list) or not properties:
                continue
            normalized_properties: list[str] = []
            for prop in properties:
                property_name = str(prop or "").strip()
                if property_name:
                    normalized_properties.append(property_name)
            if not normalized_properties:
                continue
            selected_map[entity_type] = normalized_properties
        ...
        for entity in full_schema:
            ...
            if properties != ["all"]:
                selected_names = set(properties)
                for field_name in ("static_property", "dynamic_property"):
                    definitions = entity_copy.get(field_name, {})
                    ...
                    for name, definition in definitions.items():
                        ...
                        elif field_name == "dynamic_property" and name == "default_property":
                            retained[name] = definition
                    entity_copy[field_name] = retained
