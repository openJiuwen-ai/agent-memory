...
| `extractor_impl/` | Extractor 实现目录（keyword / llm / dynamic_llm / video_memory，以及显式启用的 entity_schema）；entity_schema 为 Property 产出稳定实体分组 key、Source 时间上下文与事件精度/区间；video_memory 将视频规约结果转换为 CLM/ELM |
...
`schema_enabled=true` 时，`IndexBuilderProducer` 会在任意具体
IndexBuilder target 外包装 Entity → Property MemoryUnit ID 的 KV 派生反向索引维护器；
写入、更新和硬删都经同一包装处理，软删保留关联以支持属性版本历史。
...
15. **Schema 时间投影保留精度与来源语义**
    ...
    as-of 上下文，但不得当成事件时间。相对时间必须绑实际
    `source_unit_ids` 对应的 Source 日期。

16. **Schema 实体反向索引是派生数据**
    `schema_enabled=true` 时，由 `IndexBuilderProducer` 包装任意具体 target
    统一维护。`FORWARD_ONLY build` 记录排除项，`FORWARD_ONLY update` 保留已有
    membership。Property canonical identity
    依次取 `schema_entity_id`、`schema_entity_key`、`type::name`，并要求非空
    `schema_property_name`。首次启用时按整个 Scope 一次性回填，再以 Scope watermark
    提交完整状态；失败必须使 watermark 失效并让读侧回退。软删保留 membership 以支持
    属性版本历史，携带 MemoryUnit 的硬删和 `remove_with_scope()`
    均同步维护 membership/pointer。manager 没有 KV 端口时不包装，读取侧使用
    DomainStore Scope fallback。

17. **Schema Identity 与 Property Merge 顺序固定**
    Extractor 只生成临时观察 key；Evolver 必须在任何 Property 写入前运行 Entity
    Resolver，写入 canonical entity id，然后 Property Planner 才能按该 id 召回与合并。
    Registry 是可重建派生投影，Property MemoryUnit 是事实真源。显式 delete 不落盘，
    通过 `IndexBuilder.update(FORWARD_ONLY)` 加 `remove(SOFT)` 归档命中旧事实。时间缺失、
    格式非法或与 value 矛盾必须进入 Schema 纠错重试，不得静默清空后接受。
