...
- 规则：
  ...
  - 对信息片段按如下标准分为三类
    1. 用户本人画像信息(user_profile)：Step 2中主体锁定为{{subject_scope}}，且是关于**{{subject_scope}}**的{{user_profile_definition}}
...
- 规则：
  - 对于【用户本人画像信息】分类：{{subject_naming_rule}}，每条信息必须是单值陈述句：用户的[属性][系动词][值], 用户[动词][内容]（例："用户的职业是xx"、"用户不喜欢xx"）
  - 对于【情景记忆信息】分类：{{subject_naming_rule}}，**必须**将所有相对时间（下周、上周、上个月、昨天等）按照Step 3的推理结果替换为绝对日期，**禁止**在输出中保留任何相对时间词。时间范围用"-"连接（如"用户在2026年1月6日-2026年1月12日之间去北京出差"）
