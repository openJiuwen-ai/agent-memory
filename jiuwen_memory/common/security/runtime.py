...
装配逻辑）随本实装 PR 落地。旧加密模块 :mod:`common.security.security` 的公开名称保留
一个发布周期的导入兼容；旧 YAML 在配置解析期迁移后，Producer 无冲突接管
``security`` 顶层段。
"""
...
class SecurityRuntimeProducer(Factory):
    ...
    # 旧 ``SecurityProducer`` 注册在同一顶层段 ``security`` 下的 target。正常配置解析会
    # 自动迁移旧 local；此守卫覆盖绕过解析器直接构造 ComponentConfig 的调用路径。
    _REMOVED_CRYPTO_TARGETS = frozenset({"local"})

    @classmethod
    def build(cls, target: str, params, ctx, *, name: str = ""):
        """拦截未经过兼容迁移的旧加密配置，给出明确迁移指引。
