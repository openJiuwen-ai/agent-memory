...
原因带出，由宿主折算成它自己的结论类型（F03 决策 4 的编码约束一）。

**步骤按目标次序编排**，不照抄控制层现有实现的次序：后者是「主体覆盖 → 组织边界」，
而正确次序要求组织边界在前，以使跨组织请求得到准确的拒绝原因（F03 决策 4 的编码约束
二）。本模块覆盖十步中的第 6、7、10 步；第 8 步主体覆盖的结果由宿主算出后传入，
第 1、3、4、5、9 步在本特性内无判据来源，见 F03 决策 4「五步无判据来源」。
"""
...
# 判定输入中经属性通道传递的四个键名（S09「空间事实的两层投影与传入通道」）。
...
class DenyReason(str, Enum):
    ...
    拒绝原因的取值是审计与告警的匹配依据，属封闭集合，本规约不扩充它。这里只定义 S09
    明确指派的两项。

    跨组织拒绝复用 ``NOT_COVERED``，由 ``rule`` 取 ``cross_org`` 在审计侧区分：S09 要求
...
def _owner_entry_grade(actor: Scope, facts: SpaceAuthorizationFacts, entry: str) -> str:
    ...
    if entry in OWNER_ENTRY_SAME_DIMS:
        ...
        return ""
    if entry in OWNER_ENTRY_COVERS and any(covers_owner(o, actor) for o in facts.owners):
...
def governance_grade(actor: Scope, facts: SpaceAuthorizationFacts) -> SpaceGovernanceRole:
    """调用方在该空间的治理档，即成员记录授予上界的基数（S09「改写防护与三处上界」）。
    ...
    治理轴最高档）无判据来源，见 F03 决策 4「五步无判据来源」。
    """
