...
"""进程内令牌桶限流，按调用方地址分桶（F05 §Protection §入口限流）。
...
1. **多副本各算各的**：进程内计数，N 个副本 = N 倍实际额度。故
   ``supports_distributed_quota()`` 保持默认的 ``False``——真正的多副本限流要
   Redis 之类的共享计数器，届时在 ``protection_impl`` 下新增一个实现并覆写该
   capability，中间件不用改。
2. **按地址分桶挡不住僵尸网络**：来源足够分散时每个 IP 都拿到一个新满桶。
   能收敛这种攻击的是对昂贵校验本身的全局并发预算（见
   :mod:`common.security.protection.workload_guard`），那是与限流互补的另一层。
"""
