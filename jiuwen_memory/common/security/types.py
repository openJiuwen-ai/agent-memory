...
class AuthContext:
    ...
    **ROOT 由 ``role`` 表达，不由 actor 的形状表达**（F05 §授权不变量 1）。旧实现的
    ...
    ``credential_issuer`` 是签发凭据的具名 Authenticator 实例标识，供撤销注册表区分
    多套平行 API Key 真源；``auth_method`` 仍只表达认证协议。是否必须在线复核撤销由
    ``credential_status_required`` 显式声明，不能从 ``credential_id`` 是否非空推断：
    ROOT key 与 trusted gateway 同样需要稳定指纹做审计，但不属于 PrincipalKeyStore。
    """
    ...
    credential_issuer: str = ""  # 签发凭据的具名 Authenticator 实例
    credential_status_required: bool = False  # True 时 PEP 必须在线复核撤销状态
    ...
    **威胁边界**：此 HMAC 方案仅防止跨进程伪造（_ORIGIN_KEY 是进程内随机密钥）。
    ...
    payload = {
        "auth": {
            ...
            "credential_status_required": auth.credential_status_required,
...
class RequestSecurityContext:
    ...
    def has_valid_origin(self) -> bool:
        """Return whether all security fields still match the controlled origin proof."""
...
class Grant:
    ...
    ``grant_id`` 是服务端生成的审计标识：撤销、审计与告警都按它定位。构造时
    默认留空，由管理面写入 ``GrantStore`` 时生成；这也保持了旧公共 API 的
    ``Grant(grantor=..., grantee=..., actions=...)`` 构造形状。

    ``actions`` 在值对象边界统一冻结。旧调用方传入的 ``list[Action]`` 会立即
    归一为 ``frozenset[Action]``，非法成员则在构造阶段报错，不能把类型错误推迟
    到 ``GrantStore`` 或授权判定路径。
    """
    ...
    def __post_init__(self) -> None:
        try:
            ...
            raise TypeError("Grant.actions must be an iterable of Action values") from exc
        if not all(isinstance(action, Action) for action in actions):
            raise TypeError("Grant.actions must contain only Action values")
