...
class RoutingAuthorizer(Authorizer):
    ...
    def is_test_only(self) -> bool:
        """A routing PDP is test-only when any reachable delegate is test-only."""
        return any(policy.is_test_only() for policy in self._policies.values())
    ...
    def _delegation_stores(self):
        # P1-3：与 grant stores 同理——空间链复核委托须读全部可达 policy 的
        # DelegationStore，否则「委托写入 A policy 的 Store、请求路由到 B policy」
        # 就复核不到。去重口径与 management_grant_stores 一致。
        stores = []
        seen_ids = set()
        for policy in self._policies.values():
            for store in _delegation_sources_for(policy):
                store_id = id(store)
                if store_id not in seen_ids:
                    stores.append(store)
                    seen_ids.add(store_id)
