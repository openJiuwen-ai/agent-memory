...
``control`` 的历史数据；PR2 生产判定不再读取旧表，不以 PR3 为旧链路退场前提。
"""
...
def _backend_errors(method):
    """Translate sqlite failures into the stable authorization backend error."""
    ...
    def _wrapper(self, *args, **kwargs):
        try:
            ...
            raise BackendError(f"authorization store unavailable: {exc}") from exc
...
CREATE TABLE IF NOT EXISTS auth_delegations (
    ...
    allowed_spaces_encoding TEXT NOT NULL DEFAULT 'legacy',
...
def _load_spaces(raw: str, encoding: str) -> frozenset[str]:
    if encoding == "legacy":
        # 旧逗号格式无法区分单个含逗号名称与多个空间，不猜测，也不丢弃空项。
        if "," in raw:
            raise BackendError("ambiguous legacy delegation spaces; reseed required")
        return frozenset({raw}) if raw else frozenset()
    if encoding != "json-v1":
        raise BackendError("unknown delegation spaces encoding")
    try:
        values = json.loads(raw)
    except (ValueError, TypeError):
        raise BackendError("invalid delegation spaces encoding") from None
    if not isinstance(values, list) or any(not isinstance(value, str) for value in values):
        raise BackendError("invalid delegation spaces encoding")
...
class _SQLiteBacked:
    ...
    def __init__(self, db_path: str) -> None:
        ...
        with self._lock:
            ...
            columns = {
                row["name"] for row in self._conn.execute("PRAGMA table_info(auth_delegations)")
            }
            if "allowed_spaces_encoding" not in columns:
                self._conn.execute(
                    "ALTER TABLE auth_delegations ADD COLUMN "
                    "allowed_spaces_encoding TEXT NOT NULL DEFAULT 'legacy'"
                )
        ...
        with self._lock:
            self._conn.execute(
                ...
                ON CONFLICT(grant_id) DO UPDATE SET
                    grantor_org=excluded.grantor_org,
                    grantor_space=excluded.grantor_space,
                    grantor_user=excluded.grantor_user,
                    grantor_agent=excluded.grantor_agent,
                    grantor_session=excluded.grantor_session,
                    grantee_org=excluded.grantee_org,
                    grantee_space=excluded.grantee_space,
                    grantee_user=excluded.grantee_user,
                    grantee_agent=excluded.grantee_agent,
                    grantee_session=excluded.grantee_session,
    ...
    def _get_for_revoke(self, grant_id: str) -> Grant | None:
        with self._lock:
            row = self._conn.execute(
                "SELECT * FROM auth_grants WHERE grant_id=?", (grant_id,)
            ).fetchone()
        if row is None:
            return None
        return Grant(
    ...
    def _revoke_bound(self, grant_id: str, grantor: Scope) -> None:
        with self._lock:
            result = self._conn.execute(
                "UPDATE auth_grants SET revoked_at=COALESCE(revoked_at, ?) WHERE grant_id=? "
                "AND grantor_org=? AND grantor_space=? AND grantor_user=? "
                "AND grantor_agent=? AND grantor_session=?",
                (_iso(_now()), grant_id, *_scope_values(grantor)),
            )
            if (
                result.rowcount == 0
                and self._conn.execute(
                    "SELECT 1 FROM auth_grants WHERE grant_id=?", (grant_id,)
                ).fetchone()
                is not None
            ):
...
class SQLiteDelegationStore(_SQLiteBacked, DelegationStore):
        ...
        with self._lock:
            self._conn.execute(
                ...
                INSERT INTO auth_delegations (
                    ...
                    actions, expires_at, not_before, allowed_spaces, allowed_spaces_encoding,
                    bound_credential_id, bound_session, created_at, revoked_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(delegation_id) DO UPDATE SET
                    delegator_org=excluded.delegator_org,
                    delegator_space=excluded.delegator_space,
                    delegator_user=excluded.delegator_user,
                    delegator_agent=excluded.delegator_agent,
                    delegator_session=excluded.delegator_session,
                    delegate_org=excluded.delegate_org,
                    delegate_space=excluded.delegate_space,
                    delegate_user=excluded.delegate_user,
                    delegate_agent=excluded.delegate_agent,
                    delegate_session=excluded.delegate_session,
                    actions=excluded.actions,
                    expires_at=excluded.expires_at,
                    not_before=excluded.not_before,
                    allowed_spaces=excluded.allowed_spaces,
                    allowed_spaces_encoding=excluded.allowed_spaces_encoding,
                    bound_credential_id=excluded.bound_credential_id,
                    bound_session=excluded.bound_session,
                    revoked_at=excluded.revoked_at
                ...
                (
                    ...
                    *_scope_values(delegation.delegate),
                    ...
                    json.dumps(sorted(delegation.allowed_spaces), ensure_ascii=False),
                    "json-v1",
    ...
    def get(self, delegation_id: str) -> Delegation | None:
        ...
        if row is None:
            return None
        spaces = _load_spaces(row["allowed_spaces"], row["allowed_spaces_encoding"])
        return Delegation(
