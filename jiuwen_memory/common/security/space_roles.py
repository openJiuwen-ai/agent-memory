...
ENTRY_RULES: dict[str, EntryRule] = {
    ...
    # 摄入任务入队前的 pre-flight 鉴权，镜像 add 的鉴权路径而不落盘，故取同轴同动作。
    # 取值必须与 add 一致：本表判宽会让无权限请求占住队列，判严会拒掉有权限的摄入。
    "check_write": EntryRule(SpaceAxis.CONTENT, SpaceAction.WRITE),
...
def _in_user_dim(member: SpaceMemberFact, actor: Scope) -> bool:
    """user 维候选：agent 维为空，且 user 维与 actor 相同或为空（组织通配记录）。"""
    if member.scope.agent:
        return False
    return not member.scope.user or member.scope.user == actor.user


def _in_agent_dim(member: SpaceMemberFact, actor: Scope) -> bool:
    """agent 维候选：agent 维与 actor 相同，带 user 维时该维也须与 actor 相同。

    首个条件同时排除了两维皆空的组织通配记录——通配只在 user 维参与命中。
    """
    if not member.scope.agent or member.scope.agent != actor.agent:
        return False
    return not member.scope.user or member.scope.user == actor.user
...
def most_specific(
    ...
    记录形态跳变。
    """
    predicate = _in_user_dim if dim == "user" else _in_agent_dim
    candidates = [member for member in members if predicate(member, actor)]
    return max(
        candidates,
