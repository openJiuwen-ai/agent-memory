...
| `security/` | 安全域唯一归属地：F05 契约层（`types.py` 公共值对象、`authentication/` / `cryptography/` / `protection/` 各能力 base、`runtime.py`）+ 旧 `SecurityProvider` 横切接口（接口 + `local` ENC1 AES-GCM 实现）。`bootstrap` 已提供认证中间件与 MCP 凭据提取接缝；新 F05 的 `*_impl` 后端、SecurityRuntime Producer 与 Server lifecycle 接线仍待合入（见 `docs/features/common/F05-security-api-contracts.md`） |
...
- 安全域契约与接入中间件（认证/密码学/保护的抽象接口、公共安全值对象，以及 bootstrap 认证/传输凭据接缝；后端实现仍待合入）
- 错误类型
- 工具函数
...
11. `security/` 是 F05 安全域的契约层：消费方只 import 契约与值对象，不反向 import；bootstrap 已可消费认证/限流/工作负载保护接口，但 `*_impl` 后端、SecurityRuntime Producer 与完整 Server lifecycle 接线尚未合入。旧 `SecurityProvider` 继续从包顶层导出，新契约异常从各能力子包取。
