...
`jiuwen_memory/<subdir>/AGENTS.md` 写**可执行的规则**（不变量、禁止项、文件关系导航）与**当前实现列表**，不写意图描述。代码是 source of truth，与代码不一致时必须当场修。目的是让 AI 辅助工具读取后能快速理解当前目录下的内容。描述模块内部结构与本地约束，**不描述跨模块边界**（边界在 `docs/specs/`）。
...
### jiuwen_memory/\<subdir\>/AGENTS.md 骨架
