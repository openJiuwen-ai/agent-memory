...
def _builtin_model_client(provider, client_config: ModelClientConfig, model_config: ModelRequestConfig):
    ...
    if provider == ProviderType.OpenRouter.value:
        from jiuwen_memory.foundation.llm.model_clients.openrouter_model_client import OpenRouterModelClient
    ...
    if provider == ProviderType.SiliconFlow.value:
        from jiuwen_memory.foundation.llm.model_clients.siliconflow_model_client import \
    ...
    if provider == ProviderType.DashScope.value:
        from jiuwen_memory.foundation.llm.model_clients.dashscope_model_client import \
    ...
    if provider == ProviderType.InferenceAffinity.value:
        from jiuwen_memory.foundation.llm.model_clients.inference_affinity_model_client import \
    ...
    if provider == ProviderType.DeepSeek.value:
        from jiuwen_memory.foundation.llm.model_clients.deepseek_model_client import \
            DeepSeekModelClient
    ...
    if provider == ProviderType.IntelliRouter.value:
        from jiuwen_memory.foundation.llm.model_clients.intelli_router_model_client import \
