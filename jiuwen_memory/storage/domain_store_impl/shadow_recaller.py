...
与 KeywordRecaller/VectorRecaller 的分工：那两者取 ``storage.fulltext()`` /
``storage.vector()``（KV 时代端口，文档模式为 None → recall 返空）；本算子取
``storage.shadow_index()``（文档模式必有端口），是文档模式下唯一真正命中的召回路。
"""
...
class ShadowRecaller(Recaller):
    ...
    def __init__(self, storage: StoreManager) -> None:
        ...
        self._shadow: DocumentShadowIndex = storage.shadow_index()
        # 持有 manager 供后续生产过滤点读真源（is_retrieval_candidate）使用——与
    ...
    # 生产过滤（lifecycle/时间/标签）不在本算子做——检索侧三重排除已覆盖：
    # ① 影子索引把 build_system_filters 产出的 lifecycle/t_valid/t_invalid/t_event
    #    谓词编译进 SQL WHERE（_compile_system_filters，索引级排除）；
    # ② retriever 三条 pipeline 物化后统一过 is_retrieval_candidate 复核
    #    （pipeline_retriever._materialize_recalled/_filter_materialized 与
    #    CompositeStorage.retrieve 的 _passes）；
    # ③ 调用方契约「先 lifecycle.transition 再 remove(SOFT)」保证投影列已同步。
    # 构造器持有的 self._storage 预留点读真源能力，供后续 L2 实体扩展时使用。
...
def _build(config):
    return ShadowRecaller(StoreManagerProducer.resolve(config))
