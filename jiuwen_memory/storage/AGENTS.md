...
| 文件 | 职责 |
|---|---|
| `storage.py` | Storage 统一契约与 StorageProducer：MemoryUnit 领域操作（`add`/`update`/`delete` 带 `include_forward`/`only_forward` 意图参数）、能力发现、底层端口与检索适配入口。原文不在此列——它不是存储领域概念，由构建层注入独立 `KVStore` 自行读写 |
