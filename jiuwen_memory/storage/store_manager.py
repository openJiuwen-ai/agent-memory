...
ENTITY 端口（F07）是第七 capability，与其余六类同构暴露；两处例外须知：其一，方法首
...
全局唯一 manager（F07）：进程内共享一个 StoreManager，由 ``globals.store_manager``
...
class StoreManagerProducer(Factory):
    ...
    TOP_NAME 为 ``"store_manager"``（F07 起顶层配置段从 ``storage:`` 更名
    ...
    def resolve(cls, config) -> StoreManager:
        ...
        全局唯一 manager 语义（F07）：指名实例须在 store_manager 命名空间声明，或经
