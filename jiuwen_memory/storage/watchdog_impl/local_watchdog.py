...
class LocalWatchdog(Watchdog):
    ...
    构造期注入 shadow + markdown_root + scope + loop + debounce；``start`` 在
    事件循环就绪后调（实现内 ``asyncio.get_running_loop`` 取 loop——构造期 loop
    参数为 None 时 fallback 到 start 时取）。shadow 经 Storage 端口注入（读写同源），
    markdown root 经 MarkdownStore.root 契约读，二者与 CompositeStorage 共享同一实例。
    """
    ...
    def started(self) -> bool:
        """是否已启动（Observer 在跑）——契约 ``Watchdog.started``。"""
    ...
    def start(self) -> None:
        ...
        if self._observer is not None:
            return  # 幂等：已启动
        # stop 后再次 start 须复位：否则 _schedule_sync/_arm_timer/_fire_sync 全被
        # _closed 拦截，observer 线程重建但事件全部丢弃（半僵尸）。
        self._closed = False
...
def _build(config):
    """从装配好的 Storage 端口构造 LocalWatchdog。

    与 ShadowRecaller / DocumentIndexBuilder 同范式：shadow 与 markdown 一律从注入的
    Storage 取端口，不自行 ``build_named("default")``——保证与 CompositeStorage 读写同源，
    且 markdown_store / shadow_index 换具名实例不再静默断链。markdown root 从
    ``MarkdownStore.root`` 契约读（ConfigSource 晚绑定解析后的值），不再跨 namespace 摸
    ``params.root`` 字面量。``loop`` 不在此注入——``start`` 时由
    ``asyncio.get_running_loop`` 取（装配期无 running loop）。
    """
