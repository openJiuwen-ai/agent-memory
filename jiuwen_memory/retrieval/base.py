...
→ ④ 渐进式披露 → ⑤ 返回 + 检索轨迹。②的单路召回由数据面承担
（:class:`~storage.domain_store_impl.recaller.Recaller`，非本层算子），
其余环节拆成可插拔算子：
...
``RetrievalOperatorType`` 不含 RECALLER：单路召回是数据面的内部件，由
``CompositeDomainStore`` 组装持有，不进检索算子表。
"""
