...
| `engine_impl/cloud_engine.py` 装配类型 | `CloudEngineDependencies` 聚合共享算子、DomainStore 与建树 Job 专用 KV；`CloudEngineOptions` 聚合消息类型与默认 pipeline 配置，既有 YAML 配置键不变 |
...
19. **演进任务与内部构建请求分离**：公开任务使用 `EvolveTaskOptions`，Engine 与各 Job
    ...
    CloudEngine 为建树 Job 单独注入 `hierarchy_kv`，只转交同源 KV 端口，不用它执行
    Engine 的 MemoryUnit 点读或列表读取；这些读取继续经 DomainStore。
