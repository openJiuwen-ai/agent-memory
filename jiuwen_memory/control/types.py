...
class SpaceSpec:
    ...
    # （F03 决策 4）。该入口回落改造前的判据，实际持有者是运维通道。
...
class SpaceInfo:
    ...
    # 归属主体登记（S09）：一个没有任何成员记录的空间在判定链看来无人可放行，本字段
...
class SpaceFacts:
    """一次调用内的空间授权事实快照，由 API 层取一次、贯穿判定与谓词生成（S09）。
...
class PermissionContext:
    ...
    metadata: dict[str, str] = field(default_factory=dict)
    # 空间授权事实快照，由鉴权点一次读取后折算传入（S09「空间事实的两层投影与传入通道」）。
