...
| `hierarchy_candidates.py` | 建树只读收集器：完整分页、精确 Scope 身份、旧父全部直接子叶补齐与保护限额 |
...
1. **运行时构建依赖同源**：各 Spec 不在装配期解析 Evolver/IndexBuilder；Engine
   ...
   CloudEngine 将专用于建树任务的 `hierarchy_kv` 作为运行时 KV 注入，常规读取仍使用
   DomainStore；InMemoryEngine 继续注入自己的 KV 端口。
...
3. **先收齐候选再写**：HierarchyJob 只从 `/memory/` 收集 ACTIVE TIME snapshot 与
   home 中相交旧 time_span，并补齐旧父全部直接子叶；不按 infer 过滤，不读取 messages。
   Scope 边界必须在子引用点读前检查。分页漂移、超限、缺子或非法双向引用均失败，
   不截断后调用 Composer。身份键必须保留完整五维 Scope 与 id。
4. **只支持显式两层一次性建树**：HierarchyJob 固定 interval=0，任务 Scope 等于
   tree_home_scope；不扩大到其它 kind/角色链，也不自行触发周期、写后或召回时建树。
   任务可跨 session 收叶，实际 time_span 分组仍由 Composer 的 TimeSpanMerger 决定。
