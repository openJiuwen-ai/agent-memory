...
ANSWER_GENERATION_PROMPT = """You are a personal assistant with access to memories from past \
conversations with a user. Answer the question using information from the memories below. Be direct and concise.
...
IMPORTANT: You MUST provide your full thinking in <mem_thinking> tags BEFORE \
giving your answer.; Reasoning and answer:"""
...
def sort_search_results_newest_first(
    ...
    return [result for _, result in dated] + undated
...
def get_answer_generation_prompt(
    ...
    if not search_results:
        ...
        # Preserve the RRF order supplied by the evaluator. Dates are display
        # metadata only and must not reorder a selected Top-K.
        lines = []
        current_date = None
        for result in search_results:
...
JUDGE_PROMPT = """I will give you a question, a correct answer (or rubric), and a model response. \
