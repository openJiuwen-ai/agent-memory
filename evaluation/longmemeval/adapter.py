    ...
    def _dialogue_turns(turns: list[dict]) -> list[tuple[int, int, list[dict]]]:
        ...
        while index < len(turns):
            ...
            if not isinstance(current, dict):
                raise ValueError("LongMemEval turn must be a JSON object")
            next_index = index + 1
            has_next = next_index < len(turns)
            next_turn = turns[next_index] if has_next else None
            current_is_user = str(current.get("role", "")) == "user"
            next_is_assistant = isinstance(next_turn, dict) and str(
                next_turn.get("role", "")
            ) == "assistant"
            if current_is_user and next_is_assistant:
                grouped.append((index, next_index, [current, next_turn]))
                index += 2
                continue
