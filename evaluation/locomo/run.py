...
环境变量（.env 已配华为云 MaaS DeepSeek-V3.2 + BGE-M3）：
    ...
    RUN_TAG=v1          本轮的人类可读标签，只用于区分实验组
    RUN_ID=              本轮唯一标识；默认自动生成并用于产物目录和 Scope 隔离
    SCOPE_TAG=           仅 SKIP_WRITE=1 时填写旧 run_meta.json 中的同名值
    SKIP_WRITE=0         跳过写入并复用显式 SCOPE_TAG 指向的后端数据
    CLEANUP_AFTER_RUN=0  完成产物后清理本轮可检索记忆和索引；默认保留
...
if _PROJECT_ROOT not in sys.path:
    sys.path.append(_PROJECT_ROOT)  # 仓库根：jiuwen_memory/ 与 bootstrap/ 均为顶层包
load_dotenv(dotenv_path=os.path.join(_EVALUATION_DIR, "environment", ".env"), override=True)

from evaluation.locomo import prompts  # noqa: E402
from evaluation.shared.config_loader import load_layer  # noqa: E402
from jiuwen_memory.api import (  # noqa: E402
    DeleteMode,
    DeleteSelector,
    MemoryAPI,
    assemble_runtime,
...
from jiuwen_memory.retrieval.types import DisclosureLevel  # noqa: E402
...
DISABLE_THINKING = os.getenv("DISABLE_THINKING", "0").strip().lower() not in (
    "",
    "0",
    "false",
    "no",
...
# ---- 单轮跑批的产物目录 --------------------------------------------------- #
# 默认每轮产物进 outputs/locomo/<RUN_ID>_<RUN_TAG>/，而不是直接落在 locomo/ 根目录。
...
def _validated_identifier(name: str, value: str, *, max_length: int) -> str:
    if len(value) > max_length or _IDENTIFIER_RE.fullmatch(value) is None:
        raise ValueError(
            f"{name} 只能包含字母、数字、点、下划线和连字符，且最长 {max_length} 字符"
        )
    return value
...
RUN_TAG = _validated_identifier(
    "RUN_TAG", os.getenv("RUN_TAG", "v1").strip() or "v1", max_length=64
)
RUN_ID = _validated_identifier(
    "RUN_ID", os.getenv("RUN_ID", "").strip() or _new_run_id(), max_length=128
)
SKIP_WRITE = os.getenv("SKIP_WRITE", "0").strip().lower() in ("1", "true", "yes", "on")
CLEANUP_AFTER_RUN = os.getenv("CLEANUP_AFTER_RUN", "0").strip().lower() in (
    "1",
    "true",
    "yes",
    "on",
)
_explicit_scope_tag = os.getenv("SCOPE_TAG", "").strip()
if SKIP_WRITE and not _explicit_scope_tag:
    raise ValueError("SKIP_WRITE=1 必须显式设置 SCOPE_TAG")
if _explicit_scope_tag and not SKIP_WRITE:
    raise ValueError("SCOPE_TAG 仅能与 SKIP_WRITE=1 一起用于显式复用旧 Scope")
if SKIP_WRITE and CLEANUP_AFTER_RUN:
    raise ValueError("复用旧 Scope 时不能开启 CLEANUP_AFTER_RUN")
# 正常写入时自动加唯一 RUN_ID；仅 SKIP_WRITE 对比实验允许显式复用旧 Scope。
SCOPE_TAG = _validated_identifier(
    "SCOPE_TAG",
    _explicit_scope_tag or f"{RUN_TAG}-{RUN_ID}",
    max_length=240,
)
SCOPE_ORG = f"locomo-infer-{SCOPE_TAG}"
if len(SCOPE_ORG) > 256:
    raise ValueError("LoCoMo scope org 不能超过 256 字符")
_explicit_out = os.getenv("OUTPUT_DIR", "").strip()
if _explicit_out:
    _out_dir = _explicit_out
    RUN_DIR = ""
else:
    RUN_DIR = os.path.join(_RUNS_ROOT, f"{RUN_ID}_{RUN_TAG}")
    _out_dir = RUN_DIR
...
result_path = os.getenv(
    "RESULT_PATH", os.path.join(_out_dir, f"test_result{_V5_SUFFIX}.json")
)
metrics_path = os.getenv(
    "METRICS_PATH", os.path.join(_out_dir, f"test_metrics{_V5_SUFFIX}.json")
)
...
# ---- 产物落盘路径（按 conv 分文件，对齐 SSH 参考实现）--------------------
# 提取的记忆：write 阶段每批 api.write 返回的派生 MemoryUnit 序列化落盘。
mem_extract_path = os.getenv(
    "MEM_EXTRACT_PATH", os.path.join(_out_dir, f"test_mem_extract{_V5_SUFFIX}")
)
# 生成的 response：每题 qa_idx/question/answer/response/category 落盘（test_response_v5<conv>.json）
response_path = os.getenv(
    "RESPONSE_PATH", os.path.join(_out_dir, f"test_response{_V5_SUFFIX}")
)
# 召回的记忆：generate 阶段每题召回的 RetrievedItem 列表 + created_at（test_recall_v5<conv>.jsonl）
recall_detail_path = os.getenv(
    "RECALL_DETAIL_PATH", os.path.join(_out_dir, f"test_recall{_V5_SUFFIX}")
)
# 错题统计：validate 阶段只记错题（召回记忆+response+judge 输出）（test_errors_v5<conv>.jsonl）
errors_detail_path = os.getenv(
    "ERRORS_DETAIL_PATH", os.path.join(_out_dir, f"test_errors{_V5_SUFFIX}")
)
# 错题详情开关（默认开）：保存召回记忆、对错和 judge 输出，便于排查。
SAVE_ERROR_DETAIL = os.getenv("SAVE_ERROR_DETAIL", "1").strip().lower() not in (
    "",
    "0",
    "false",
    "no",
...
_current_conv_ctx: contextvars.ContextVar[int | None] = contextvars.ContextVar(
    "conv_id", default=None
)
_current_phase_ctx: contextvars.ContextVar[str | None] = contextvars.ContextVar(
    "phase", default=None
)
...
def _patched_openai_chat(self, messages, **options):
    ...
    temperature = options.get("temperature", getattr(self, "_default_temperature"))
    max_tokens = options.get("max_tokens", getattr(self, "_default_max_tokens"))
    # OpenAILLM 属性名随版本变动（旧 _model_name，新 _fallback_model），
    # 经 _endpoint() 统一解析拿当前生效 model/client，不读私有属性以免漂移。
    endpoint_resolver = getattr(self, "_endpoint", None)
    if not callable(endpoint_resolver):
        raise RuntimeError("OpenAILLM endpoint resolver is unavailable")
    ...
    merge_request_options = getattr(self, "_merge_request_options", None)
    if not callable(merge_request_options):
        raise RuntimeError("OpenAILLM request option merger is unavailable")
    merge_request_options(create_kwargs, options)
...
_META_ENV_KEYS = (
    "RUN_ID", "RUN_TAG", "SCOPE_TAG", "SKIP_WRITE", "CLEANUP_AFTER_RUN",
    "CONV_IDS", "START_CONV", "CONCURRENCY",
...
def _git_commit() -> Optional[str]:
    ...
    try:
        out = subprocess.run(
            [git_executable, "-C", _PROJECT_ROOT, "rev-parse", "--short", "HEAD"],
...
def _upstream_commit() -> Optional[str]:
    ...
    该值仍取最近一次 fetch 的本地跟踪 ref，不在评测过程中访问网络。
    """
    git_executable = shutil.which("git")
    if git_executable is None:
        return None
    try:
        for ref in ("origin/mem2.0", "upstream/mem2.0"):
            out = subprocess.run(
                [git_executable, "-C", _PROJECT_ROOT, "rev-parse", "--short", ref],
                capture_output=True,
                text=True,
                encoding="utf-8",
                errors="replace",
...
def _record_run_completion(cleanup: dict, failures: list[tuple[int, str]]) -> None:
    """将收尾状态补入现有 run_meta，不记录异常正文或任何凭证。"""
    path = os.path.join(_out_dir, "run_meta.json")
    try:
        with open(path, encoding="utf-8") as fh:
            meta = json.load(fh)
        meta["finished_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
        meta["failed_conversation_ids"] = [conv_id for conv_id, _ in failures]
        meta["cleanup"] = cleanup
        with open(path, "w", encoding="utf-8") as fh:
            json.dump(meta, fh, ensure_ascii=False, indent=2)
    except Exception as e:
        logger.warning("run_meta 收尾状态落盘失败(忽略): %s", str(e)[:120])
...
def _conversation_scope(conv_id: int) -> Scope:
    return Scope(org=SCOPE_ORG, user=f"conv_{conv_id}")


def _scope_security(scope: Scope) -> RequestSecurityContext:
    return RequestSecurityContext(auth=AuthContext(actor=scope))


def _list_scope_units(api: MemoryAPI, scope: Scope) -> list:
    offset = 0
    page_size = 100
    units = []
    expected_count: int | None = None
    security = _scope_security(scope)
    while expected_count is None or offset < expected_count:
        page = api.list(scope, security=security, offset=offset, limit=page_size)
    ...
    if expected_count is None or len(units) != expected_count:
        raise RuntimeError(
            f"LoCoMo cleanup pagination mismatch: got {len(units)}, "
            f"expected {expected_count}"
        )
    return units


def _purge_run_data(api: MemoryAPI, conv_ids: list[int]) -> dict[str, int]:
    """删除本轮可检索 MemoryUnit 及派生索引；原文缓存和审计按内核契约保留。"""
    deleted_count = 0
    scopes = [_conversation_scope(conv_id) for conv_id in sorted(set(conv_ids))]
    for scope in scopes:
        units = _list_scope_units(api, scope)
        if units:
            deleted = api.delete(
                DeleteSelector(
        ...
        remaining = _list_scope_units(api, scope)
        if remaining:
            raise RuntimeError(
                f"LoCoMo cleanup left {len(remaining)} memory units in {scope}"
            )
...
class ConversationProcessor:
    ...
    def __init__(self, conv_id: int, api, llm):
        ...
        # 正常运行使用唯一 Scope；SKIP_WRITE 只有显式指定旧 SCOPE_TAG 才允许复用。
        self.scope = _conversation_scope(conv_id)
        self.identity = self.scope  # 本人操作本人 scope
        # F05 重构后 add/search/inspect 用 security 上下文，从 auth.actor 取 identity
        self.security = _scope_security(self.identity)
        ...
        for sk in session_keys:
            ...
            for turn, chat in enumerate(chats, start=1):
                ...
                if chat.get("blip_caption"):
                    text += (
                        "(The conversation is accompanied by an image, and the "
                        f"description of the image is '{chat['blip_caption']}')"
                    )
    ...
    def recall(self, question: str, qa_idx: int | None = None) -> tuple[list, dict]:
        ...
        # 取 unit.temporal.t_event（即 write 时传入的 occurred_at，
        # session 事件时间）作为 created_at。
    ...
    def answer_and_judge(
        self, request: _AnswerJudgeRequest
        ...
        """
        sliced = request.search_results[: request.cutoff]
    ...
    def process_full(self, conv: dict) -> dict:
        ...
        _top = cutoff_accuracy.get(f"top_{_max_cut}")
        if _top is None:
            raise RuntimeError(f"missing cutoff result: top_{_max_cut}")
        ...
        for cutoff in CUTOFFS:
            cutoff_result = cutoff_accuracy.get(f"top_{cutoff}")
            if cutoff_result is None:
                raise RuntimeError(f"missing cutoff result: top_{cutoff}")
            cutoff_summary_parts.append(
                f"top_{cutoff}={cutoff_result['accuracy']['overall']:.3f}"
            )
        cutoff_summary = "  ".join(cutoff_summary_parts)
...
class TESTLOCOMO:
    ...
    def dump_metrics(metrics: dict, out_path: str) -> None:
        ...
        for p, desc in phase_meta.items():
            phase_overall = overall[p]
            logger.info("[统计] %s(%s): 调用%d次 耗时%.2fs p50=%.1fms p90=%.1fms p95=%.1fms "
                        "input=%d output=%d total=%d",
                        desc, p, phase_overall["invoke_count"],
                        phase_overall["elapsed_seconds"],
                        phase_overall["p50_ms"], phase_overall["p90_ms"],
                        phase_overall["p95_ms"], phase_overall["input_tokens"],
                        phase_overall["output_tokens"], phase_overall["total_tokens"])
        logger.info("=" * 60)
    ...
    def dump_retrieval_subprocess_latencies(
        ...
        sub_desc = {
            ...
            "tokenize": (
                "jieba 分词(query 主分词+特征抽取分词, "
                "parse 段内每次检索 2 次; 首样本含冷启动)"
            ),
        }
...
def _run(api: MemoryAPI) -> int:
    # 构造共享 LLM 实例（generate/judge 用，与引擎内 LLM 同配置；
    ...
    cleanup = {"requested": CLEANUP_AFTER_RUN, "status": "retained"}
    if CLEANUP_AFTER_RUN:
        try:
            cleanup.update(status="purged", **_purge_run_data(api, conv_ids))
        except Exception as e:
            cleanup_failed = True
            cleanup.update(status="failed", error_type=type(e).__name__)
            logger.error("本轮可检索记忆清理失败: %s", e)
    _record_run_completion(cleanup, failures)
    _finalize_run_dir()
    logger.info("完成!")
    return 0 if not failures and not cleanup_failed else 1
...
def main() -> int:
    ...
    logger.info("[assemble] 装配 MemoryRuntime ...")
    t0 = time.perf_counter()
    runtime = assemble_runtime(config=config)
    logger.info(
        "[assemble] 完成 %.2fs api=%s",
        time.perf_counter() - t0,
        type(runtime.api).__name__,
    )
    try:
        return _run(runtime.api)
    finally:
        runtime.close()
