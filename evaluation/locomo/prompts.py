# Prompt 文本严格复刻远端评测，保留原始长行以避免改变运行时内容。
# ruff: noqa: E501
...
ANSWER_GENERATION_PROMPT = """You are answering a question using retrieved memories from past \
conversations. Follow these reasoning steps IN ORDER.
...
- **For listing questions ("what X has Y done", "what are Y's X", "what kind of X")**: Before answering, explicitly enumerate a CANDIDATE LIST of every distinct item you find, scanning ALL memories line by line from first to last. Do NOT give a partial list of the top few items. A single memory may contain multiple items, and items are often scattered across many memories including ones far down the list. Think about what CATEGORIES of answers the question could have, then re-scan specifically for each category. If you found N distinct items, your final answer MUST list all N — not a subset.
...
If you found items during reasoning that you're tempted to exclude from your answer — STOP. Include them unless you have STRONG evidence they are wrong. The most common mistake is finding relevant items but then dropping them due to overly strict filtering, OR giving only 2-3 representative items when 6 were actually found. More items is better than fewer when there is supporting evidence.
- **For listing questions**: You MUST include EVERY distinct item you found in the final answer. Do NOT summarize a list of 6 items as "pottery, painting, and other activities" — list all 6 explicitly. Do NOT drop items to keep the answer concise. Do NOT collapse to a representative subset.
