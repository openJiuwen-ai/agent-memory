...
@pytest.mark.parametrize("delegated", [False, True], ids=["user", "delegated-agent"])
def test_http_coords_write_preserves_authors_landing_and_user_isolation(
    collective_http_url, method, delegated
) -> None:
    provision_spaces(collective_http_url)
    payload = {
        "scope": {"org": "local", "user": "u1"},
        "system_metadata": {"infer": "true", "coords": {"team": "t"}},
    }
    if delegated:
        payload["scope"].update(agent="a1", session="s1")
    content = "我习惯用 Python 写代码，另外我们团队规定代码评审必须两人"
    if method.startswith("batch_"):
        payload["items"] = [{"content": content}]
    else:
        payload["content"] = content
    selector = "test-u1-agent" if delegated else "test-u1"
    status, result = post_as(collective_http_url, selector, method, payload)
    ...
    for memory_class, space in (("user_pref", "u-u1"), ("team_convention", "team-t")):
        ...
        assert metadata["author_agent"] == ("a1" if delegated else "")
        ...
        read_status, stored = post_as(
            collective_http_url,
            "test-u1",
            "get",
            {
                "unit_id": unit["id"],
                "scope": {"org": "local", "space": space},
            },
        )
    ...
    for user, expected in (("u1", {"user_pref", "team_convention"}), ("u2", {"team_convention"})):
        search_status, found = post_as(
            collective_http_url,
            f"test-{user}",
            "search",
            {
                "query": "Python 评审 约定 习惯",
                "top_k": 10,
...
def test_agent_selector_cannot_impersonate_user_via_target_scope(collective_http_url) -> None:
    """DEV selector 只认证独立 agent；目标 user 不是 delegation 的替代品。"""
    provision_spaces(collective_http_url)
    status, body = post_as(
        collective_http_url,
        "test-unbound-agent",
        "add",
        {
            "content": "forged delegated write",
            "scope": {"org": "local", "user": "u1", "agent": "a1", "session": "s1"},
            "system_metadata": {"coords": {"team": "t"}},
        },
    )
    assert status == 400, body
    assert "does not match the caller identity" in body["message"]
