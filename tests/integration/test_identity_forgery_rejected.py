...
from jiuwen_memory.common.security.types import Role, reset_current, set_current  # noqa: E402
...
def srv():
    ...
    403），而「未配置 security / 未显式选择 permission」时内核已改为 DEV 兼容回退
    （``AllowAllPermissionManager``，恒放行）。隔离断言不能借用该全放行内核，须显式装
    SQLite（见第三次验收报告约束 5）。
    """
    import server
...
# (org=system, user=dev) + role=ROOT 跨 org 放行」。其前提是 ``PermissionManager``
