"""显式 DEV 业务流的回归防线：未接 PR2 前跨组织业务不断流。
...
- 而是由 Access composition root 在「入口显式选择 loopback DEV、配置未显式选择
  security/permission」时把默认权限实现换成既有
...
- ``dev_security.with_local_dev_security`` 把完整 DEV Runtime 与 ``permission.default``
  注入为 ``allow_all``——注入的是 ``config.settings["memory_api"]``
  的**副本**，不修改用户原字典；触发条件 = 入口提供 DEV runtime、该段无
  ``security`` 且无 ``permission``，任一配置存在都尊重用户选择、不注入；
- DEV 认证器产出 ``local/developer`` 主体、``role=ROOT``；
- ``PermissionManager`` 不收 ``role``：接口只有 ``(actor, target, action, context)``，
  放行与否完全由 ``Scope`` 判定。DEV 主体（``local/developer``）持具名 Scope，恒被
...
def _dev_runtime():
    """构造带 PR2/PR3 空接口占位的完整显式 DEV runtime。"""
...
def dev_srv():
    ...
    composition root 经 ``with_local_dev_security`` 注入完整 DEV Runtime 与
    ``permission.default=allow_all``（注入配置副本，不改原字典）。
    公共 ``assemble`` 不做此覆写，默认权限仍是 sqlite。
    """
    import server
    ...
    return server.Server.build(with_local_dev_security(load_config([OFFLINE])))
...
def test_dev_fallback_perm_is_allow_all_authorizer_placeholder_only(dev_srv) -> None:
    ...
    assert isinstance(dev_srv.security_runtime.authorizer, AllowAllAuthorizer)
    assert dev_srv.security_runtime.authorizer.is_test_only()
...
def test_dev_composition_root_injects_allow_all_on_copy() -> None:
    """DEV 配置适配器返回副本，不改原始 ``config.settings``。

    触发条件 = 入口显式提供 DEV runtime，且配置无 ``security`` / ``permission``。
    """
    import server
    from profiles import OFFLINE, load_config
    ...
    srv = server.Server.build(with_local_dev_security(cfg))
