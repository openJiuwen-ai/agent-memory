...
"""ENTITY 端口（StorageCapability 第七席，F07）：能力发现、命名端口、授权代理、health、装配。

F07 把 EntityStore 从旁路装配（两侧消费方各自 ``EntityStoreProducer.dep``）收进
...
def test_named_entity_ports_truth_table() -> None:
    ...
    manager = CompositeStoreManager(
        kv=InMemoryKVStore(), entity={"default": default_stub, "aux": aux_stub}
    )
...
def test_entity_ports_drop_none() -> None:
    ...
    manager = CompositeStoreManager(kv=InMemoryKVStore(), entity={"broken": None})
...
def test_health_dedupes_shared_entity_instance() -> None:
    ...
    manager = CompositeStoreManager(
        kv=InMemoryKVStore(), entity={"default": shared, "aux": shared}
    )
...
def _ctx(entity_ns: dict[str, Any] | None) -> Any:
    """装配用 ctx：端口全量来自各 ``*_store`` 命名空间，manager 段无需 params 引用键。"""
    raw: dict[str, Any] = {
...
def test_entity_ports_aggregate_whole_namespace() -> None:
    """命名空间下所有实例全量成为端口，``default`` 只是其中一个普通键。"""
    register_backends()
    ctx = _ctx(
        {
            "default": _es(hosts="http://es-default:9200", index="ents_default"),
...
def test_entity_capability_holds_without_default_port() -> None:
    """只声明具名实例（无 default）：capability 成立、具名端口可用、default 不可用。

    capability 与 has_*() 同源于端口表——「有能力但无 default 端口」是一致的状态，
    不会出现 capabilities() 说没有、has_entity(name) 说有的分叉。
    """
    register_backends()
    ctx = _ctx({"aux": _es(hosts="http://es-aux:9200", index="ents_aux")})
