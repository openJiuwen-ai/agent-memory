...
# -- 集成场景：defaults 合并后看门狗随文档开启 ------------------------------ #


def test_defaults_merged_watch_document_follows_write_document() -> None:
    """真实装配盲区：用户开 write_document 但不配 watch_document 时，defaults 合并后
    watch_document 应为 True（随文档开启），而非 False 基线泄漏把看门狗压死。

    复现 assembly.py 装配取值链：default_context().merged(user) → globals.get(..., True)
    → resolve_watch_document(...)。修复前 defaults watch_document=False 被保留 → False；
    修复后 defaults=True → True。
    """
    user_ctx = AssemblyContext.from_dict({"globals": {"write_document": True}})
    ctx = default_context().merged(user_ctx)

    raw = ctx.globals.get(WATCH_DOCUMENT_KEY, True)
    assert resolve_watch_document(raw) is True


def test_defaults_merged_explicit_watch_document_false_is_respected() -> None:
    """用户显式关 watch_document 时合并后为 False（defaults=True 被覆盖）。"""
    user_ctx = AssemblyContext.from_dict(
        {"globals": {"write_document": True, "watch_document": False}}
    )
    ctx = default_context().merged(user_ctx)

    raw = ctx.globals.get(WATCH_DOCUMENT_KEY, True)
    assert resolve_watch_document(raw) is False
