...
def test_http_accepts_normal_request() -> None:
    ...
    target scope 用 DEV 认证自己的主体（``org="system", user="dev"``）：PR1 的
    ROOT 语义还没有执行点（``Authorizer`` 归 PR2），DEV 身份在旧 ACL 下只覆盖得到
    自己的 scope。本条测的是 body 长度处理，不是判权，故不借道跨 org 请求。
    """
    httpd, port = _start_server()
    try:
        code, _ = _post(port, json.dumps({"tenant_id": "system", "scope": "dev"}).encode())
        assert code == 200
    finally:
        httpd.shutdown()
