...
def test_named_dev_actor_passes_the_boundary() -> None:
    """具名 DEV 主体本身是合法形态，正常通过校验。"""
    with authenticated(DevAuthenticator(), Credentials()) as security:
        assert security.actor == Scope(org="local", user="developer")


def test_controlled_request_id_is_shared_with_context_and_reset() -> None:
    with authenticated(
        DevAuthenticator(), Credentials(), request_id="http-edge-request-id"
    ) as security:
...
def test_empty_controlled_request_id_falls_back_to_generated_id() -> None:
    with authenticated(DevAuthenticator(), Credentials(), request_id="") as security:
        assert security.request_id
