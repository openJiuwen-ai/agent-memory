...
def test_issued_identity_is_immutable_to_original_scope_mutation(store) -> None:
    """验收第三次 P2-1：签发后改原 actor，不影响 resolve 的身份。

    Scope 现为 frozen 值对象；_Record.actor 保存的是不可变值，原对象后续修改
    （若调用方仍持旧可变引用）不会改变已签发 key 的 principal。防的是「签发后
    把 actor 改成受害者 org」的越权。
    """
    from dataclasses import FrozenInstanceError

    actor = Scope(org="tenant-a", user="alice")
    key = store.issue(actor, Role.USER)
    # 原 actor 已 frozen，无法原地改；确认身份未变
...
def test_resolved_auth_context_actor_is_deeply_immutable(store) -> None:
    """验收第三次 P2-1：resolved AuthContext.actor 不可原地修改。

    AuthContext(frozen=True) 此前只是浅冻结，actor 是可变 Scope；frozen Scope 后
    深度不可变，请求生命周期内身份不可篡改。
    """
    from dataclasses import FrozenInstanceError
