...
def test_dev_returns_root_with_named_local_actor() -> None:
    ...
    assert ctx.actor == Scope(org="local", user="developer")
...
def test_dev_actor_is_not_shared_across_authentications() -> None:
    ...
    assert first.actor == Scope(org="local", user="developer")
    # 任意一方被改写都不影响另一方，也不影响下一次认证
    first.actor.org = "attacker"
    third = auth.authenticate(Credentials())
    assert second.actor.org == "local"
    assert third.actor == Scope(org="local", user="developer")
