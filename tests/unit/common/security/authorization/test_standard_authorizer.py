...
from jiuwen_memory.common.security.authorization.store import DelegationStore, GrantStore
...
class FakeGrantStore(GrantStore):
    ...
    def find_active(self, *, grantee, grantor_org, action, now):
        ...
        return [g for g in self.grants if action in g.actions and g.grantor.org == grantor_org]
...
def test_grant_allows_cross_user_access() -> None:
    grant = Grant(grant_id="g1", grantor=BOB, grantee=ALICE, actions=frozenset({Action.READ}))
...
def test_grant_for_another_grantee_does_not_apply() -> None:
    grant = Grant(
        grant_id="g1",
        grantor=BOB,
        grantee=Scope(org="acme", space="main", user="carol"),
        actions=frozenset({Action.READ}),
    )
    decision = _decide(_authorizer(grants=[grant]), _auth(), _resource(scope=BOB))
    assert not decision.allowed
...
def test_grant_across_space_within_org_is_allowed_when_explicit() -> None:
    ...
    grant = Grant(grant_id="g1", grantor=archive, grantee=ALICE, actions=frozenset({Action.READ}))
...
def test_authorizer_does_not_read_contextvar() -> None:
    ...
    """
    from jiuwen_memory.common.security.types import reset_current, set_current
