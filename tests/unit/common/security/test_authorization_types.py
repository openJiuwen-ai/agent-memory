...
def test_grant_normalizes_actions_at_construction_boundary() -> None:
    grant = Grant(
        grantor=Scope(org="acme", user="alice"),
        grantee=Scope(org="acme", user="bob"),
        actions=[Action.READ, Action.WRITE],  # type: ignore[arg-type]
    )
...
def test_grant_rejects_invalid_action_at_construction_boundary() -> None:
    with pytest.raises(TypeError, match="only Action values"):
        Grant(
            grantor=Scope(org="acme", user="alice"),
            grantee=Scope(org="acme", user="bob"),
            actions=["read"],  # type: ignore[list-item]
        )
