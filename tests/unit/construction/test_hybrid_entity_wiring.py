...
F07 起本文件另覆盖**装配侧**：两个消费方（写入侧 hybrid、召回侧 keyword）都经
``manager.entity(name)`` 取 ENTITY 端口，不再各自 ``EntityStoreProducer.dep``；
端口未装配时降级关闭。核心用例是"读写共享同一端口"——F07 之前这靠两侧 params
...
# ---------------------------------------------------------------------------
# 装配侧（F07）：两个消费方经 manager 的 ENTITY 端口取实例
# ---------------------------------------------------------------------------
...
def test_builder_and_recaller_share_the_same_entity_port(_clean_factory) -> None:
    """F07 核心收益：读写两侧取的是同一 manager 的同一 ENTITY 端口。
