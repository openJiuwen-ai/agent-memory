...
文档模式下作为 IndexBuilder 装配方，全委托注入的 DomainStore（domain_store.add/update/
delete 内部按文档模式分流到 md.write + shadow.insert_units）。失效方向：

- 构造期不校验「非文档模式」，会让首次写入以 AttributeError 暴露而非清晰报错——真源写错
  地方。
- ``RETRIEVAL_ONLY`` 语义：文档场景影子索引即真源、无独立检索索引可补建，应整段跳过
  （连 domain_store.add 都不调）。
- build/update/remove 各委托 domain_store 一次，逐 unit 下传，mode 原样透传。
"""
...
class _RecordingStorage:
    """记录 add/update/delete 调用的假 DomainStore（文档模式）。"""

    def __init__(self, *, write_document: bool = True) -> None:
