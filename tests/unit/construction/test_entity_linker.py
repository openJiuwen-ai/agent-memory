...
def _is_entity_record_call(node: ast.AST) -> bool:
    """判断 AST 节点是否为 ``EntityRecord(...)`` 构造调用。"""
    return (
...
def test_entity_linker_does_not_project_scope_into_entity_records() -> None:
    ...
    entity_record_calls = [node for node in ast.walk(tree) if _is_entity_record_call(node)]
