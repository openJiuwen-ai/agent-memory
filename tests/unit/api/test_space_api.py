...
def test_memory_api_space_lifecycle_usage_members_and_delete() -> None:
    ...
    info = api.create_space(
        ...
        security=legacy_request_context(org_admin),
    )
    assert info.status == SpaceStatus.ACTIVE
    assert (
        api.get_space("acme", "coding", security=legacy_request_context(space_admin)).display_name
        == "Coding"
    )
    assert [
        space.space for space in api.list_spaces("acme", security=legacy_request_context(org_admin))
    ] == ["coding"]
    ...
    api.add_space_member(
        ...
        security=legacy_request_context(space_admin),
    )
    assert (
        api.list_space_members("acme", "coding", security=legacy_request_context(space_admin))[
            0
        ].scope
        == unit_scope
    )
    ...
    assert api.export_space("acme", "coding", security=legacy_request_context(space_admin))

    result = api.delete_space("acme", "coding", security=legacy_request_context(space_admin))
    assert result.deleted_counts["memory"] == 1
    assert result.deleted_counts["index"] == 1
    events = api.audit(
        {"target_space": "coding"}, security=legacy_request_context(Scope()), limit=100
    )
    assert unit.id in events[-1].detail.get("deleted_memory_ids", "")
    with pytest.raises(NotFoundError):
        api.get_space("acme", "coding", security=legacy_request_context(space_admin))
...
def test_memory_api_rejects_writes_after_space_archive() -> None:
    ...
    api.create_space(
        SpaceSpec(org="acme", space="coding"), security=legacy_request_context(org_admin)
    )
    api.archive_space("acme", "coding", security=legacy_request_context(space_admin))

    with pytest.raises(ValidationError):
        api.add("blocked after archive", unit_scope, security=legacy_request_context(unit_scope))
...
def test_space_policy_principal_path_drives_api_authorization() -> None:
    ...
    api.add(
        ...
        target,
        security=legacy_request_context(Scope(org="acme", space="coding", agent="agent-a")),
    )
...
def test_in_memory_engine_rejects_non_empty_space() -> None:
    ...
    with pytest.raises(ValidationError, match="InMemoryEngine"):
        api.add("cloud scoped memory", scope, security=legacy_request_context(scope))
