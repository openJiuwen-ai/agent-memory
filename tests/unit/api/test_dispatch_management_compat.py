...
def _sqlite_server():
    """显式装 SQLite permission 的 Server（保留用户/属主隔离）。

    「未配置 security / 未显式选择 permission」时内核回退到 AllowAllPermissionManager
    （恒放行），故测**权限拒绝**的用例必须显式装配 SQLite，不能借 DEV 兼容回退断言
    403（第三次验收报告约束 5）。
    """
    return Server.build(
        load_config(
...
def test_dispatch_admin_requires_platform_admin_under_default_kernel() -> None:
    # admin 经 PermissionManager 的 platform-admin 放行线：非 root 主体须拒。
    # 显式装 SQLite（而非 DEV 兼容回退）——这条断言的是「平台管理面要 root 主体」，
    # 不能借恒放行的 AllowAll 绕过权限门。
    srv = _sqlite_server()
...
def test_dispatch_revoke_denied_for_non_owner() -> None:
    ...
    显式装 SQLite：非属主撤销是权限拒绝，不能借 DEV 兼容回退绕过。
    """
    srv = _sqlite_server()
