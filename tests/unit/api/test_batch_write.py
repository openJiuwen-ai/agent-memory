...
def test_batch_write_normalizes_defaults_and_preserves_input_order() -> None:
...
def test_batch_write_collects_item_validation_errors_and_continues() -> None:
    api = build_kernel().api
    scope = Scope(org="acme", user="alice")
...
def test_batch_write_fail_fast_marks_remaining_items_skipped() -> None:
    api = build_kernel().api
    scope = Scope(org="acme", user="alice")
...
def test_batch_write_rejects_duplicate_sequence_within_scope_and_stream() -> None:
    api = build_kernel().api
    scope = Scope(org="acme", user="alice")
...
def test_batch_write_authorizes_each_item_without_blocking_later_owner_item() -> None:
...
def test_batch_write_async_matches_synchronous_result_shape() -> None:
    api = build_kernel().api
    scope = Scope(org="acme", user="alice")
