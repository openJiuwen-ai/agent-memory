...
def test_create_registers_the_reverse_index_entry() -> None:
    ...
    assert manager.index.spaces_for(Scope(org="acme", user="alice"), "acme") == ("u-alice",)
...
def test_legacy_single_axis_roles_are_mapped_on_read() -> None:
    ...
    manager.create(SpaceSpec(org="acme", space="team"))
    legacy = [
        {"scope": {"org": "acme", "space": "team", "user": u}, "role": role}
        for u, role in (("o", "owner"), ("a", "admin"), ("m", "member"), ("v", "viewer"),
                        ("x", "unknown-role"))
    ]
    kv.insert(
        _scope("acme", "team"),
...
def test_remove_member_clears_the_record_and_its_index_entry() -> None:
    ...
    assert manager.index.spaces_for(Scope(org="acme", user="alice"), "acme") == ()
...
def test_add_member_rolls_back_the_member_table_when_the_index_write_fails() -> None:
    ...
    def _boom(principal: Scope, space: str) -> None:
        raise BackendError("index unavailable")

    manager.index.add = _boom  # type: ignore[method-assign]
