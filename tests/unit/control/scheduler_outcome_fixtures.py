...
async def finish_job(scheduler: Scheduler, job: OutcomeJob) -> JobInfo:
    ...
    async with asyncio.timeout(3):
        ...
        await wait_for_job_completion(job)
        await finish_background_tasks()
    return scheduler.status(submitted_id)


async def wait_for_job_completion(job: OutcomeJob) -> None:
    """Wait safely when the scheduler runs the Job on its own thread and event loop."""
    if not await asyncio.to_thread(job.completed.wait, 3):
        pytest.fail("Job did not signal completion within 3 seconds")
