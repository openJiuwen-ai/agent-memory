...
from memory_core.manage.mem_model.memory_unit import MiddleTermUnit, MemoryType
...
async def test_add_middle_term_memory_success():
    ...
    assert len(docs) == 1
    assert docs[0][0] == "middle-mem-001"  # mem_id
...
async def test_add_middle_term_memory_wrong_type_filtered():
    ...
    from memory_core.manage.mem_model.memory_unit import SummaryUnit
...
async def test_add_middle_term_memory_with_real_data():
    ...
    docs = call_args.kwargs["docs"]
    assert docs[0][0] == "mid-20260623-001"
