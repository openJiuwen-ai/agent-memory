...
_CONFIG = load_config([OFFLINE] + [load_layer(p) for p in sys.argv[1:]])
_MEMORY_API_CONFIG = _CONFIG.settings.get("memory_api") or {}
if not is_http_transport(_TRANSPORT) and not _MEMORY_API_CONFIG.get("security"):
    # stdio 没有远端监听面：在 PR2 接入前保留既有“零配置即可本地使用”流程。
    # HTTP 传输不走该兼容层，未配置认证时按 required 模式逐请求拒绝。
    _CONFIG = with_local_dev_security(_CONFIG)
    logging.getLogger("agent-memory.mcp").warning(
        "MCP stdio is using development authentication; credentials are ignored"
    )
_SRV = Server.build(_CONFIG)
...
def main() -> int:
    ...
    if transport in ("http", "streamable-http"):
        ...
        if _SRV.authenticator is not None and _SRV.binding_policy is not None:
            try:
