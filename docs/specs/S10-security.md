...
6. **具名系统主体**：为兼容上游既有开发环境及本地数据，DEV 固定产出
...
DEV 时，才用配置适配器补齐完整 `SecurityRuntime`；MCP stdio 为兼容本地工具启动，会在未配
安全段时显式套用同一适配器，MCP 的 HTTP 传输不回落 DEV。
...
在 PR2 Authorizer 尚未接管 `role=ROOT` 的过渡期，只有 HTTP/CLI 的显式 DEV 入口和 MCP
stdio 本地入口可以先调用 `with_local_dev_security()`，向 `memory_api` 配置的副本注入
`permission.default=allow_all`：

1. composition root 已显式选择本地 DEV，且用户没有配置 `security` 段；
