...
> 本节按 §2 的类型视角重组：通用/自主记忆底座 → Code Agent / IDE 助手 → 企业级 → 个人助手 → 框架绑定，便于按场景阅读。
...
### 1.5 OpenViking（字节跳动 / 火山引擎 Volcengine）
...
### 1.9 OpenAI Codex 记忆（Codex CLI / App）
...
### 1.15 AgentScope Memory + ReMe（阿里巴巴）
