...
| 影响范围 | jiuwen_memory/storage/kv_impl/encrypted_kv_store.py，jiuwen_memory/storage/kv_impl/__init__.py，jiuwen_memory/storage/fs_impl/encrypted_fs_store.py，jiuwen_memory/storage/fs_impl/__init__.py，jiuwen_memory/common/security/cryptography/，docs/specs/S06-storage.md，docs/features/common/F04-security-interfaces-and-encryption.md |
| 测试基线 | KV 侧：`tests/unit/storage/test_encrypted_kv_store.py` 覆盖加密写入、读后解密、scan 解密、透传操作、工厂装配与失败关闭。FS 侧：`tests/unit/storage/test_encrypted_fs_store.py` 13 条全绿；单元全量 `15 failed, 814 passed, 1 skipped`，15 个失败全为预存在的环境失败（14 个 `test_jieba_tokenizer.py` 缺 `nlp` extra，AUTH-ENC-04 修复后该断言在 Windows 上改为校验 DACL：icacls 禁继承 + 仅当前服务账号一条显式授权，与 POSIX 的 `0o600` 各自成立）。相关模块测试、ruff 与 `git diff --check` 已通过 |
| Refs | — |

> **2026-08-05 F05 迁移后记。** 本文记录的是 2026-07 的决策过程，正文保留原貌。
> 三处已被 F05 Common Security 推翻或改名，以下述为准：
>
> 1. **`EncryptionProvider` → `CryptographyProvider`**，落点从 `jiuwen_memory/common/encryption/`
>    迁到 `jiuwen_memory/common/security/cryptography/`；`EncryptionContext` → `CryptoContext`，
...
因此加密能力需要落在 KV 边界：对调用方保持 `KVStore` 合同不变，对底层后端只写入密文。算法、密钥来源、明文兼容策略不归 storage 层管理，而是由 `jiuwen_memory/common/encryption/` 的 `EncryptionProvider` 提供。
...
`EncryptedFSStore` 做的事和 `EncryptedKVStore` 逐条对应：构造 `CryptoContext` 与 AAD，转发给注入的 `CryptographyProvider`。密码学一行都不在 storage 里。

依赖方向 `storage → common.security.cryptography`，单向；cryptography 不认识 Store。回归防线：`test_encrypted_fs_is_registered_by_storage_bootstrap`。

### 10. 装饰器住在 `jiuwen_memory/storage/`，不住在 `jiuwen_memory/common/encryption/`
...
- **在 MemoryAPI/add/search/get 中分别调用 encryption**：被拒。上层入口太多，且未来新增 engine 或批处理入口时容易遗漏；KV 装饰器可以把加密收敛到单一边界。
...
- ~~**根密钥文件权限在 Windows 上设不出 `0o600`**~~：已修复。`_restrict_file_mode`
  在 Windows 上改用 DACL（`icacls /inheritance:r` + 仅当前服务账号）实现等价语义，
  POSIX 维持 `chmod 0600`；测试在两个平台各自断言真实权限，不再按 `os.name` 跳过。
