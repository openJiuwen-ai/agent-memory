...
原协议；已有未完成操作仍按恢复规则重试。实体索引开关不参与 Schema 能力判断。
...
旧证据防回放同时检查 Schema Source 输入和候选 ADD 入口。更新记录关联旧 Source ID、
正文及修订标记；多源更正额外记录对应属性的旧来源修订。不同属性及真正的新证据仍可写入。
每次显式更新分配新的 Source 修订标记，区分 A→B→A 的最新 A 与原始 A 的回放。

提交前冻结操作 ID、候选 ID、前后快照和全部动作，写入私有恢复记录；每步成功后保存进度。
提交时重读输入，发现已变化则拒绝；索引失败通过 `PartialFailureError` 暴露，同一目标和
patch 重试复用冻结动作。实体索引的严格错误反馈通过 ContextVar 限定到 Schema 提交，
并在成功或失败时恢复，普通写入继续沿用增强索引失败只记日志的行为。

完成后压缩恢复记录，移除正文快照，留下 ID、哈希和防回放关系；API 审计记录操作 ID 和
涉及的 Unit ID。恢复提供可重试流程，不承诺跨后端原子事务。
...
提交范围内仅保留现有 Schema 测试存储的 `scan()` 接口适配；新增更新回归、主线隔离回归
及夹具按评审范围要求保留在本地。因此下列含本地用例的结果不能仅检出分支复现。
...
事实、完整空结果、歧义、权限、跨 pipeline 迁移、配置失效、截断/超时、提交中断后重建
协调器重试、完成后重试和正文改回旧值。
...
- 未完成操作需使用相同目标、patch 和相容配置重试。同 Scope 存在未完成的 Schema
  更新时，其他 Schema 正文更新先报冲突；没有后台自动恢复任务。
- 恢复可靠性取决于所注入 KV 的持久性；纯内存部署的进程退出不会保留状态。
- 防回放只覆盖可识别的旧 Source 修订；把旧正文换成全新 ID 重新导入，或历史数据丢失
  修订信息，不保证识别为旧证据。
- 当前按 Scope 扫描关联 property 和恢复关系，大规模部署可增加专用反向索引；本次
  不宣称完成性能优化。
