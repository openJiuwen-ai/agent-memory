...
| 测试基线 | Schema 定向测试 33 passed；Storage + Construction 回归 355 passed、5 skipped |
| Refs | — |
...
`schema_orchestrating` Evolver；`build_schema_kernel()` 再委托官方
`build_kernel()` 完成
...
`SchemaOrchestratingEvolver` 只覆盖官方 `_evolve_extract()`：
...
原始输入
  ...
  -> 再调用 Schema Extractor
       ...
       抽取失败：记录 schema_extract_degraded，保留 Source MemoryUnit
       Entity Identity 异常：丢弃未解析派生，保留 Source MemoryUnit
```
...
Merge LLM 重试耗尽时，同名候选回退 UPDATE，否则回退 CREATE；UPDATE 目标不存在时也转为
CREATE，并在属性处理前生成正式 entity id。Resolver 实例发生非预期异常时，不把 provisional
property 或 entity observation 交给 Property Merge/Registry，而是放弃本轮 Schema 派生；
Source-first 已保存的原文继续可检索。显式关闭 Entity Identity 时保留原有 provisional-key
兼容路径。
...
但仍执行 Entity Identity、Entity Registry 和 Property Merge。Schema property 绕过普通
相似度 Dedup，交给 Property Merge。官方 `DynamicEvolver` 继续保留给非 Schema 普通记忆；
本扩展不再提供重复的 `schema_dynamic` 包装器。

### 5. 与 mem2.0 数据结构兼容

Schema 内部字段写 `system_metadata`，来源业务字段按 mem2.0 规则继承到
`user_metadata`；实体明文写入 `MemoryUnit.entities`。不新增或修改官方 `MemoryUnit` 字段。
Schema 派生构造 `system_metadata` 时只继承非 Schema 上下文，来源中的旧 `schema_*`、
`property_merge_*`、`extraction_mode`、`target` 等内部状态先剥离，再由本轮候选重新生成，
避免旧实体身份、事件时间和合并结果污染新事实。
...
本扩展默认关闭。官方 `assemble()` 不注册 Schema target，内置默认配置仍选择
`dynamic_llm` Extractor、`orchestrating` Evolver 和 `composite` Storage。调用方需要从
`jiuwen_memory.schema` 使用 `assemble_schema()` 注册扩展，并显式配置默认 extractor 为
`entity_schema`、默认 evolver 为 `schema_orchestrating`，Schema 才会进入写入链路。写入时设置
...
- `test_entity_schema_extension.py`、`test_schema_property_merge_extension.py`、
  `test_schema_composite_storage.py`：33 passed；
- `tests/unit/storage` 与 `tests/unit/construction`（排除未被本特性修改的
  `test_entity_linker.py`）：355 passed、5 skipped；跳过项为当前环境未安装的可选
