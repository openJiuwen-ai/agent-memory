...
|---|---|
| 日期 | 2026-08-05 |
| 影响范围 | `bootstrap/core/`、`src/control/`、`src/ingest/`、`src/construction/`、`src/retrieval/` |
| 当前状态 | 初版实现，端到端装配与验证尚未完成 |
...
| 后台任务管理 | `src/control/ingest_job.py` | 管理异步执行、任务状态和提交幂等 |
| 视频规约 | `src/ingest/normalizer_impl/` | 执行视频处理并生成 clips/events 结构化结果 |
| 多层级记忆构建 | `src/construction/extractor_impl/video_memory_extractor.py` | 将 clips/events 转换为 CLM/ELM `MemoryUnit` |
| 多模态记忆检索 | `src/retrieval/retriever_impl/multimodal_retriever.py` | 判断 scope 是否存在多模态记忆，并行召回原生记忆、CLM 和 ELM 后融合 |
...
**输入输出**：输入视频 URI、scope、`payload_id` 和 metadata；输出 `ing_` 前缀的
`job_id`，由 `/v1/job` 返回后续状态和记忆结果。

**职责**：`handler.py` 识别 `modality=video`，校验请求并把原生写入调用交给 Control；
`server.py` 只负责装配和关闭 `IngestJobController`。Bootstrap 不持有线程池、任务状态
或幂等规则。
...
**代码路径**：`src/control/ingest_job.py`

**输入输出**：输入 `payload_id`、视频 URI、scope 和待执行任务；输出 `IngestSubmission`，
并维护 `pending -> running -> succeeded/failed` 状态。

**职责**：`IngestJobController` 管理线程池、队列容量、任务状态、幂等和状态持久化。
...
- `src/common/normalizer/normalizer_impl/routing_normalizer.py`
- `src/common/normalizer/normalizer_impl/video_normalizer.py`
- `src/common/normalizer/normalizer_impl/video_pipeline.py`
...
- `src/construction/extractor_impl/video_memory_extractor.py`
- `src/construction/evolver_impl/orchestrating_evolver.py`（原生实现）
...
- 已构建多模态记忆：同时检索原生记忆、片段级 CLM 和事件级 ELM；各通道独立返回
  结果，最后统一融合。
...
MemoryAPI.recall(query, scope)
  -> MultimodalRetriever 判断 scope 是否存在有效多模态记忆
     ...
     -> true：
        -> 原生文本记忆召回 -> text_hits
        -> 按 modal_type=multimodal、memory_level=clm 召回 CLM -> clip_hits
        -> 按 modal_type=multimodal、memory_level=elm 召回 ELM -> event_hits
...
文本与视频记忆使用同一个 `MemoryUnit`。视频差异主要体现在 `segments`、`metadata`
和 `provenance`。
...
| `metadata.modal_type` | 无固定值 | `multimodal` |
| `metadata.memory_level` | 无固定值 | `clm` 或 `elm` |
...
精简文本，在 `metadata` 下的 `memory_level` 里标注是 `clm` 还是 `elm`。
