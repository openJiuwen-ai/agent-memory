...
静态检查修正将组件解析留在 Engine 类内，共享更新逻辑只调用注入的 Evolver 和 IndexBuilder 解析回调，不跨类读取受保护字段，也不新增普通请求的路由工作。复杂条件拆成短路判断，审计 ID 收集、变更分组和回放过滤改为显式循环，保留原顺序、去重与有效期语义。接入配置加载直接从公开 API 导入必需的 ValidationError，避免无隔离的动态导入。
...
静态检查修正后，原失败的导入隔离检查、配置加载、接入 API 边界及本地 Schema 更新/隔离回归共 123 passed、2 skipped；两项跳过为本地 Engine 不适用的云端迁移用例。另对所报函数做 AST 结构检查，确认条件最多三个布尔子句、推导式逻辑简单，共享 Schema 更新模块不再访问 Engine 的受保护成员；该检查不代替服务端静态检查器。

扩大到 construction/control/api/common 与配置加载用例的本地回归为 1519 passed、5 skipped、6 failed；六项失败分别来自 BGE-M3 的两项用例缺少 torch，以及出站 SSL 的四项用例缺少 httpx。此轮不宣称全量回归通过，相关依赖与失败位置均未在本次静态修正中修改。
