...
| 项 | 值 |
|---|---|
| 编号沿革 | security/F01 -> common/F07 -> common/F09 -> common/F10（先让位 F07-temporal-t-message，现让位上游 F09-log-privacy） |
| 日期 | 2026-08-17 |
| 实施阶段 | 认证与加密期（对应 [F04](F04-security-interfaces-and-encryption.md) §术语说明中的"认证与加密期"） |
| 影响范围 | `jiuwen_memory/common/security/authentication/`、`jiuwen_memory/common/security/cryptography/`、`jiuwen_memory/common/security/protection/`、`jiuwen_memory/common/security/{types.py,runtime.py}`、`jiuwen_memory/storage/kv_impl/`、`jiuwen_memory_entry/core/auth_middleware.py`、对应镜像测试目录与各 surface 装配入口 |
| 测试基线 | 以本分支门禁结果与下方验证清单为准；FS 加密测试不属于 PR1 |
...
> - **ROOT 具名主体（IMPL-01，2026-08-27 落实）**：dev 为兼容上游既有行为产出
>   `Scope(org="local", user="developer")`，Root API Key 产出
>   `Scope(org="system", user="root")`，权限只由 `role=ROOT` 表达。
...
>   HTTP / CLI / MCP 显式固定 DEV 入口调用 `with_local_dev_security()`，在用户
>   同时未配置身份映射、`security` 与 `permission` 时向配置副本注入完整 DEV Runtime 与
>   `permission.default=allow_all`；`Server.build` 本身不识别 mode，公共
...
>   的空间、成员、读写隔离逻辑。管理员具名为 ops，认证 actor 不接受 user+agent。
>   映射 DEV 不注入 allow_all；具名角色组织权限与 agent 代表 user 的受控委托留待 PR2。
>   不把原业务成功断言改成 403，不以空主体/双主体恢复隐式特权。
> - **PR2 必须统一接管而非平行接线**：所有入口统一认证产出 RequestSecurityContext，
>   MemoryAPI PEP 调用 Authorizer。旧空间内容轴/治理轴、成员角色、作者归属、路由及
>   检索过滤都须覆盖。行为验证完成后，旧 PermissionManager 链、DEV allow_all 注入、
>   legacy 桥一起退场；PR3 审计接口不提前激活。上游多身份测试的原成功断言作为
>   PR2 验收目标；PR1/PR2 之间不发布版本。
>
...
> FS 加密装饰器及其文件大小边界已从 PR1 拆出，保存在 `sec/fs-encryption`，待独立 PR
> 评审时再归档设计与验证基线，不能在本 PR 文档中声明已经落地。
...
> [S10 §注册与配置](../../specs/S10-security.md)**：安全能力由顶层 `security` 段组合，
...
| HTTP / CLI 默认 `required`，未配置认证时拒绝 | 所有依赖隐式 DEV 的客户端 | 本地测试显式选 `--auth-mode dev`，其他部署配置认证并携带凭据 |
...
| `Server.__init__` 新增 `security_runtime` 参数 | 直接构造 `Server(...)` 的代码 | 带默认值 `None`；未装配认证时请求 fail-closed |
...
**当前默认**：HTTP / CLI 未配置 `security` 时使用 `required` 并 fail-closed；只有显式本地
DEV 才装配 `local/developer` ROOT，并继续受统一 BindingPolicy 约束。MCP（含 stdio）
同样默认 required，不再零配置回退 DEV；HTTP 映射模式按测试标识选择具名身份。
