...
>   HTTP / CLI 显式 DEV 与 MCP stdio 本地入口调用 `with_local_dev_security()`，在用户
>   同时未配置 `security` 与 `permission` 时向配置副本注入完整 DEV Runtime 与
...
DEV 和 MCP stdio 零配置兼容入口才装配 `local/developer` ROOT，并继续受统一 BindingPolicy
约束。
