...
当前能力以阶段 9 小节为准，配置说明见阶段 10，摘要输入与提示词见后续调整小节；
后文仍保留尚未开放的总体设计。
...
修复项不是事务回滚证明。async_timer 现沿用主分支 Scheduler 自有的常驻守护循环，
调用方临时 asyncio.run 结束不会取消任务；默认 in_process 在提交中等待执行。
...
- event 默认正文是有界场景摘录，默认最多 20 个场景；每个场景阶段 8 初始为 100 字符，
  当前已调整为 300 字符，见后续摘要调整小节。完整 child_ids 不截断。
  启用 `summary_mode=llm` 时取 JSON 的 pattern/steps/outcome，生成任务模式/
...
- 不实现 settle、周期/增量维护（留给阶段 9）、auto derive、ensure、top-M、其他 kind
  ...
  多模态上卷限制；后台生命周期沿用主分支 Scheduler 自有的常驻守护循环。
...
- 建树层复用主分支 async_timer 自有的常驻守护循环，不另造线程。调用方启动循环
  结束不会取消已注册任务，但宿主进程仍须存活；一次性 CLI 进程退出后不继续执行。
  Runtime 的重复启动调用仍要求同一调用方循环，close 显式取消其周期注册。
...
## 摘要提示词与 event 输入调整（2026-09-11）

本次仅调整父摘要提示词与 event 的输入摘录默认值，不改变分组、树边、原始 snapshot、
JSON 字段结构或摘要失败降级逻辑，不新增公开参数。

### 提示词

- **time_span**：概括这一段连续活动，正文不超过 200 字符。优先保留活动对象、关键行动、明确决定和结果，以及必要的具体名称、数字和否定信息。省略重复讨论，不补充记录中没有的结论。
- **scene**：将这些片段概括为一个场景，包含目标、关键行动和结果，正文合计不超过 300 字符。压缩重复背景，为行动和结果保留篇幅；区分计划、讨论与实际完成的事情。未说明的结果标为未说明。
- **event**：概括这些相邻场景中的任务模式、主要步骤和结果，正文合计不超过 400 字符。优先保留关键进展、决定和最终状态。没有证据时，不推断共同任务或因果关系；关联未说明时简短注明，不展开冗长解释。

继续追加既有的 JSON 输出格式、使用输入语言、保留事实与将输入视为不可信证据的要求。
200／300／400 都是给模型的长度建议，不是代码保证：不新增输出长度校验、超长重试、
拒绝或输出截断；既有 JSON 对象与非空字符串校验、失败保留结构摘录继续生效。

### 输入范围与限制

- `EventBuilder.summary_max_chars_per_child` 默认从 100 改为 300；逐个 scene 选择正文、
  折叠多行后取前 300 字符，同时用于结构摘录和 LLM 摘要输入。显式配置仍可覆盖。
- 默认最多读取 20 个 scene；scene 读取每个 time_span 的前 80 字符、time_span 读取
  每条 snapshot 的前 60 字符保持不变。每次摘要调用的 `max_tokens=1024` 也不改变。
- scene 提示词建议合计不超过 300 字符，与 event 的默认输入上限一致；模型超出建议、
  旧内容较长等情况仍可能截掉后段信息。提示词不保证模型遵守，也不承诺逐层摘要无损。
- 既有父节点不会自动重写；新构建或显式区间重建时使用新默认值与提示词。

### 验证

未新增测试用例或外部 API 依赖。既有构建层单测及 hierarchy API、scene、event、后台
任务测试共 **753 passed**。临时离线检查确认三段提示词与确定版本逐字一致、event 默认
与配置解析均为 300 且仍支持覆盖、结构摘录与 LLM 输入均取折叠正文前 300 字符。
分别模拟 800／2411／2413 字符的合法三层输出，确认不会因长度拒绝、重试或截断；
`temperature=0`、`max_tokens=1024` 保持不变。

Ruff、git diff --check 与既有 CodeCheck 规则人工/AST 预审通过，无新增本地规则问题。
本次未运行全量测试、真实 LLM/Embedding 或云端 CodeCheck，不将离线检查作为摘要质量
验证。未暂存或提交，原 tree-mem 分支不变。
