...
class Authorizer(ABC):
    ...
    def routing_fields(self) -> tuple[str, ...]:
        """本实现据以**选择策略**的 :class:`ResourceDescriptor` 属性名（默认不路由）。

        路由型实现按资源的某个属性挑选 delegate，而写查询的那些属性（``memory_type``
        等）源自请求。若不同时约束查询能触达的数据，调用方就能「用 A 的钥匙开 B 的
        门」：路由值填宽松策略对应的类型、``filters`` 却指向受严格策略保护的数据。
        PEP 据此把路由值**回注为系统谓词**，使授权依据与数据范围绑定。

        与 :meth:`is_test_only` 同为 capability 声明：PEP 问的是「你按什么路由」，
