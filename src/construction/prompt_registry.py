...
metadata 只写 prompt 的 **key**（引用 yml 里的命名 prompt），运行时由本注册表
按 ``phase + key`` 取回真实文本。三步共享同一注册表实例。
"""
...
class PromptRegistry:
    ...
    构造后只读；缺 key 返回 ``None``，由调用方决定回退行为。
    """

    def __init__(self, prompts: Mapping[str, Mapping[str, str]] | None = None) -> None:
    ...
    @classmethod
    def from_dict(cls, data: Mapping[str, Any] | None) -> "PromptRegistry":
        """从 yml 解析出的 ``prompts`` 段构造。"""
