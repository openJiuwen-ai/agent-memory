...
| `query_parser.py` | QueryParser 接口：查询理解（改写/分词/实体/向量化/时间解析） |
...
| `query_parser_impl/` | QueryParser 实现目录 |
...
2. 并行 Recaller[i].recall(scope, parsed_query, top_k) → list[list[ScoredUnit]]
     ↓ 多路召回
3. Fuser.fuse(parsed_query, candidates) → list[ScoredUnit]
     ↓ 融合 + 可选重排
4. UnitReader 点读 MemoryUnit → 有效性过滤（lifecycle != FORGOTTEN）
     ↓
5. Discloser.disclose(...) → list[RetrievedItem]
     ↓ 按层级加载内容（L0/L1/L2）
