...
def register_operators() -> None:
    ...
    if _REGISTERED:
        return
    import_module("retrieval.discloser_impl")
    import_module("retrieval.fuser_impl")
    import_module("retrieval.query_parser_impl")
    import_module("retrieval.recaller_impl")
    import_module("retrieval.retriever_impl")
    _REGISTERED = True
