...
- :class:`Config`：门面，``from_yaml`` / ``from_dict`` 解析，``context()`` 取 :class:`AssemblyContext`。
- :class:`AssemblyContext`：全局命名空间（top_name -> name -> RawSpec）+ 跨切面 ``globals``。
- :class:`ComponentConfig`：传给各 ``_build`` 的 ``config`` 视图（本实例 params 回退 globals + ctx 句柄）。
- :class:`RawSpec`：一个具名实例的纯数据。
- :func:`default_context`：内置默认装配上下文（纯内存离线栈，复刻共享拓扑）。
