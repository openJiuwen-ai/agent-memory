...
或自动触发 gitcode-issue-resolver。

用法:
    # 前台运行
    python issue_poller.py --config gitcode-config.json

    # 后台运行
    nohup python issue_poller.py \
        --config gitcode-config.json &
...
class IssuePoller:
    ...
    def _handle_event(
        ...
        if etype == "assign":
            ...
            print(f"[issue-poller] Issue #{number} 被标记为 [{label}]: \"{title}\"")
        elif etype == "mention":
            matched = event.get("matched", "")
            print(f"[issue-poller] Issue #{number} 评论中提到 {matched}: \"{title}\"")

        cmd = f'claude "/gitcode-issue-resolver {number}"'
    ...
    def _trigger_resolver(number: int) -> None:
        ...
        try:
            ...
            subprocess.Popen(
                [
                    claude_path,
                    f"/gitcode-issue-resolver {number}",
                ],
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--auto-trigger",
        action="store_true",
        help="自动触发 claude gitcode-issue-resolver",
    )
    parser.add_argument(
        "--once",
        action="store_true",
