    ...
    for space in backfiller.spaces_of(org):
        ...
        if len(info.owners) > 1:
            findings["multi_owner_spaces"].append(
                f"{space}: {[_render_scope(o) for o in info.owners]}"
            )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    p_backfill = sub.add_parser("backfill", help="四项回填")
    ...
    if args.command == "backfill":
        backfiller = Backfiller(
            ...
            tag_keys=kernel.api._route_table.tag_keys,
            dry_run=args.dry_run,
        )
        spaces = [args.space] if args.space else backfiller.spaces_of(args.org)
        for space in spaces:
            print(backfiller.backfill_space(args.org, space).render())
        return 0
