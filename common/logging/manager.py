...
from common.logging.protocol import LoggerProtocol
...
class LogManager:
    ...
    def reset(cls) -> None:
        ...
        try:
            from common.logging import reset_lazy_loggers
        except ImportError:
            return
        reset_lazy_loggers()

        try:
            from common.logging.events import reset_common_logger_cache
        except ImportError:
            return
    ...
    def _get_default_logger_class(cls, backend: Optional[str] = None) -> Type[LoggerProtocol]:
        ...
        if cls._default_logger_class is None:
            try:
                from common.logging.default.default_impl import DefaultLogger
    ...
    def _get_logger_class_for_backend(cls, backend: str) -> Type[LoggerProtocol]:
        ...
        if backend_name == "loguru":
            try:
                from common.logging.loguru.loguru_impl import LoguruLogger
            except ImportError as e:
    ...
    def _get_log_config(cls) -> Optional[object]:
        try:
            from common.logging.log_config import log_config
