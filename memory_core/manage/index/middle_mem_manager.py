...
from memory_core.common.base import generate_idx_name, parse_memory_hit_infos
from memory_core.manage.index.base_memory_manager import BaseMemoryManager
from memory_core.manage.mem_model.memory_unit import MiddleTermUnit, BaseMemoryUnit, MemoryType
...
class MiddleTermMemoryManager(BaseMemoryManager):
    ...
    async def _add_summary_memory_to_mem_store(self, user_id: str, scope_id: str, middle_term_unit: MiddleTermUnit):
        ...
        mem = BaseMemoryManager.encrypt_memory_if_needed(key=self.crypto_key,
                                                         plaintext=middle_term_unit.content)
        data = {
            'id': middle_term_unit.mem_id,
            'user_id': user_id or '',
            'scope_id': scope_id or '',
            'mem': mem,
            'source_id': middle_term_unit.message_mem_id,
            'mem_type': self.mem_type,
            'timestamp': middle_term_unit.timestamp
        }
        await self.mem_store.write(user_id=user_id,
                                   scope_id=scope_id,
                                   mem_id=middle_term_unit.mem_id,
                                   data=data)
    ...
    async def _add_middle_term_memory_to_vector(
        ...
        return await semantic_store.add_docs(
            docs=[(middleterm_unit.mem_id, middleterm_unit.content, middleterm_unit.timestamp)],
